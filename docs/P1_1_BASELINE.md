...
4. Auth still double-calls `getUser` (P1.3). — **P1.3:** reduced to one verified `getUser` per Owner request; the proxy now performs cookie maintenance only ([P1_3_EVIDENCE.md](P1_3_EVIDENCE.md) §1). **P1.5:** deployed-runtime confirmation obtained — every authenticated Owner document request in production emitted **exactly one** `owner_authentication` span, capability routes emitted **zero**, and proxy cookie maintenance never appeared as a second verified Owner-authentication operation ([P1_5_EVIDENCE.md](P1_5_EVIDENCE.md) §3). **Stated precisely:** this confirms the instrumented operation count, **not** the underlying Supabase `getUser` network-call count, and production log retention is demonstrably incomplete. The exact Auth HTTP count remains proven locally by `apps/web/e2e/specs/owner-shell-auth.spec.ts`.
