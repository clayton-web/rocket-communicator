...
Governed by [PROJECT_CONSTITUTION.md](PROJECT_CONSTITUTION.md). Terms: [GLOSSARY.md](GLOSSARY.md). AI law: [AI_CONSTITUTION.md](AI_CONSTITUTION.md). Binding A8 engines: **D102–D110 (A8.1)**, superseding parts of D095–D101, in [DECISIONS.md](DECISIONS.md). **Owner-controlled Task reminders** are product-authorized by **D152** (superseding in part D102/D110); they are **not implemented**.

## Objective

Private Android-first **AI Communication Action Assistant** and the Owner's **trusted external memory** (P2.0 / D138): it captures Owner communications, proposes Task Suggestions, requires Owner approval before Tasks and Recipient handoffs, delivers work via Capability Links, runs the **Follow-up Engine** and **Event Notification Engine**, records outcomes, and learns Owner preferences—without becoming a conventional task manager, calendar manager, inbox replacement, or permanent communication archive. Reminders are one capability within that system.

**Deadline and reminder are separate (D152):** a Task may have zero or one deadline and zero or multiple Owner-controlled reminders; either may exist without the other. **Current implementation:** an explicitly selected Task due date may drive deterministic **Recipient** follow-through via the A8 Follow-up Engine (D102–D110 operative engine rules). **Approved product direction:** Owner-created Task reminders at Owner-selected dates/times are permitted and are **not yet implemented**. Escalation ladders, Owner CC ladders, silent AI-controlled scheduling, and general calendar management as the product's purpose remain excluded.
...
WhatsApp, Messenger, Signal; call recording / live-call transcription; historical SMS import; replacing Messages or Phone; automatic client replies; multiple Gmail accounts; Play Store; Rocket PM; Neon; FCM unless later justified (D017); permanent archive; full Recipient dashboard; second Authenticated User; escalation ladders / Owner CC ladders; arbitrary recurrence as a calendar product; general calendar management; Owner snooze as a Follow-up control (D101); silent AI-invented or AI-scheduled reminders (D152). **Also excluded from the initial A8 slice (D110 sequencing; capability product-authorized by D152):** Owner-created additional reminders and their routes/UI/schema, reminder-time picker, preset reminder choices, Recipient reminder preferences, Android reminder UI — not product-law-forbidden, but **not implemented** and not to be built inside unauthorized A8 work.
...
- **Task due date** is optional and, when explicitly selected by the Owner, is the **authoritative deterministic scheduling input** for the A8 Recipient Follow-up Engine (D102). It is an organization-local **calendar date** with no Owner-selected time in that engine (D103). This supersedes D098 (`dueAt` informational only). **Owner-controlled Task reminders** independent of deadlines are product-authorized (D152) and **not implemented**.
- **Follow-up Engine / Event Notification Engine (A8):** authoritative **current-engine** rules in [WORKFLOWS.md](WORKFLOWS.md) §10 and D102–D110. Reminders in that engine are **Task-scoped** (D104) and send at **09:00 organization-local**: one on the calendar day before the due date (D105), then one each calendar day after it while incomplete, bounded at **14 successful overdue deliveries per schedule generation** (D106). Recipient reminders vs Owner event notifications remain separate; no escalation CC ladder (D099). Owner Event Notifications are delivered by email via the Owner’s connected Gmail; FCM/push remains deferred (D017). Waiting suspends and is the only pause mechanism (D097, D107). AI recommends; deterministic rules send (D027). **Production reminder delivery is gated** on the Event Notification Engine plus the minimum Owner schedule-status UI (D108). D102–D110 describe this engine; they are **not** a product-law bar on future Owner-controlled multi-reminders (D152).
