...
| **Proposal / candidate**                                               | Shared backend     | `TaskSuggestion` and the shared Owner-review / Keep-Assign path (**D161**, D155). A6 Application Suggestion Engine is a **preserved legacy automatic producer** (**D163**), not the carrier to extend for new capabilities | No `CandidateTask`, inbox-task persistence, or second proposal store. One interpretation occurrence → 0..N TaskSuggestions                      |
...
| **Interpretation**                                                     | Shared backend     | `packages/ai` — Owner/shared `InterpretationResult` is the future product job; A6 `SuggestionExtractionResult` remains a distinct legacy job sharing transport where appropriate (D085, **D161**, **D163**)                | No per-source implementation of "what constitutes a task." Source-specific **adapters** may differ; the semantics may not                       |
