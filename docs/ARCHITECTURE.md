...
| **Interpretation occurrence**                                          | Shared backend     | `interpretation_runs` / `InterpretationRun` in `@aicaa/db` — completed-occurrence persistence foundation (idempotency + successful outcomes only) (**D161**). Shared application producer wiring is authorized by **D169** and implemented in S3.1. **D170** authorizes S3.2 Owner `POST /api/v1/manual-captures` as the first HTTP adapter, now implemented. **D171** authorizes S3.3 Android capture-to-proposal client reachability, now **implemented**. S3 for Owner manual capture is complete at the locked capture-to-proposal boundary; full proposal-lifecycle UI remains unauthorized | No “one interpretation forever per source.” Occurrence is not canonical Task truth                                                                  |
...
**Android manual capture now runs the AI-first path (D154, D171).** Today: Capture → Save → `POST /api/v1/manual-captures` → shared interpretation → zero or more canonical TaskSuggestions → Android displays them **read-only**, with IME speech-to-text filling the same field. The product target for manual typed or dictated capture is Owner input → interpretation occurrence → zero or more proposed Tasks → Owner review → accept and choose the responsible person, **Me** or a **Recipient** → canonical Task → follow-through (D161, **D164**); the review-and-accept half of that target is **not built**. **D169** authorizes controlled S3 backend machinery for that path (S3.1: shared interpretation application service + canonical persistence). **D170** authorizes S3.2 Owner manual-capture HTTP/API (`POST /api/v1/manual-captures` + generated contracts) calling that service, implemented on the backend. **D171** authorizes S3.3 Android capture-to-proposal integration — switch capture onto that route, freeze the durable retry tuple, display returned proposals read-only, and keep encrypted device-side pending-capture retry state for at most 24 hours — and that slice is **implemented**. S3 for Owner manual capture is therefore **complete** at this locked boundary. S3.3 does **not** authorize full proposal-lifecycle actions (approve, responsibility selection, edit/merge, inbox, Task creation from proposal), and none exist. Evolving the one shared proposal path and the one shared interpretation capability (D157) remains required; do not add a second one. The Android direct-create classes (`CaptureTaskUseCase`, `TaskOwnerRepository.createCapturedTask`) remain present but unreachable from the Capture UI, kept for rollback and later cleanup, and the backend `POST /api/v1/tasks` endpoint is unchanged.
