...
| **Proposal / candidate**                                               | Shared backend     | `TaskSuggestion` and the Application Suggestion Engine (D080–D085, **D161**)                                                                                                                    | No `CandidateTask`, inbox-task persistence, or second proposal store. One interpretation occurrence → 0..N TaskSuggestions |
| **Interpretation occurrence**                                          | Shared backend     | `interpretation_runs` / `InterpretationRun` in `@aicaa/db` — inert completed-occurrence persistence foundation (idempotency + successful outcomes only; no producer wired yet) (**D161**)       | No “one interpretation forever per source.” Occurrence is not canonical Task truth                                         |
| **Interpretation**                                                     | Shared backend     | `packages/ai` — distinct A6 `SuggestionExtractionResult` and Owner/shared `InterpretationResult` jobs sharing transport where appropriate (D085, **D161**)                                      | No per-source implementation of "what constitutes a task." Source-specific **adapters** may differ; the semantics may not  |
...
| **Recipients / assignment**                                            | Shared backend     | `Recipient`, `TaskAssignment`, `TaskCapability`, `HandoffAttempt`, Gmail handoff                                                                                                                | No client-specific People list, assignment truth, or contact store                                                         |
...
**Interpretation semantics (D154, D161).** The target shape for every source is:

```
source → interpretation occurrence → zero or more proposals → Owner review → canonical Task
```
...
- Owner-initiated idempotency uses unique `(organizationId, idempotencyKey)` plus request-fingerprint conflict detection. Source identity and trigger are provenance, not uniqueness constraints.
...
TaskSuggestion is the **mutable operational proposal head**. Append-only proposal revisions preserve revision 0 (AI as presented to the Owner) and later Owner edits; the finally accepted revision must be identifiable (D155). Approval creates an **unassigned** Task (D080). **Keep** is an affirmative Owner decision recorded as D155 acceptance evidence and is never inferred from missing TaskAssignment; **Assign** uses the existing Recipient / TaskAssignment / Gmail handoff path, and assigned acceptance evidence is written only after assignment succeeds.
...
**AI and suggestions.** The Application Suggestion Engine is separate from Gmail sync (D084): heuristic relevance, then LLM extraction via `packages/ai` (D085). Approve creates an unassigned Task only (D080). Recommendations never silently become assignments, emails, or Reminder Schedules. `proposedRecipientHint` may map to `proposedRecipientId` only via deterministic match to an active Recipient (D094) — never auto-handoff. AI may recommend a **due date**; only explicit Owner selection has scheduling effect, and AI may never create, activate, alter, or suppress a Reminder Schedule (D027, D102, D152).

**Automatic email interpretation is current implementation, not the product commissioning target.** Email interpretation is targeted to become Owner-initiated ("Review with Rocket") with sender exclusion; automatic interpretation of non-excluded mail is a **future** mode (D156). The three categories are kept distinct in [WORKFLOWS.md](WORKFLOWS.md) §1a. Neither statement retracts the other, and nothing here authorizes a change to the operational path.

## Reminder and notification architecture

Authoritative rules: [WORKFLOWS.md](WORKFLOWS.md) §10. Product law: D102–D110, D128–D136, D152. Persistence reference: [packages/db/README.md](../packages/db/README.md).
