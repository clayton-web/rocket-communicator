...
**Follow-up Engine / Event Notification Engine (A8; D102–D110 supersede parts of D095–D101). Not operational — the domain logic (A8.2), persistence foundation (A8.3a), and Owner reminder APIs (A8.3b) exist; nothing schedules, sends, or is enabled in Production.** Authoritative rules in [WORKFLOWS.md](WORKFLOWS.md) §10.
...
- **One lock order across all three mutations.** Establishment, generation change, and removal each begin by locking the Task row, then write the schedule, then the Task's due date. The original ordering differed between removal and the other two and deadlocked under contention, with the victim escaping unmapped as a 500. A single row lock removes the cycle by construction rather than asking every future transaction to remember a sequence, and it makes the loser of a race read the winner's committed state and report a truthful 412. A serialization refusal is still mapped to the typed concurrency error as defence-in-depth. Proven with two connections against PostgreSQL 16; PGlite serializes on one connection and cannot express the failure.
- **Task-lifecycle coupling is not wired yet, and it gates A8.4a.** A8.3b stops a schedule on due-date removal, and its eligibility gate stops a terminal or Waiting Task from _acquiring_ a live one — but neither reacts to a transition, so a schedule established while a Task was open stays `active` after that Task completes. Waiting suspension and resume, and stopping on completion or dismissal, are deferred: Waiting is reachable by both the Owner and a Recipient capability through the shared `persistCapabilityAction` transaction, and implementing one of D107's lifecycle rules without the others would be actively wrong. Nothing sends, so this is currently inert — but no worker may scan or claim until it is closed. See the deferred lifecycle gate in [MILESTONES.md](MILESTONES.md).
