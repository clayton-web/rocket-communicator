...
Acceptance also carries **one responsibility selection** — the Owner or a Recipient (D164) — recorded as affirmative D155 evidence independent of the accepted content revision. Selecting a Recipient uses the existing Recipient / `TaskAssignment` / Gmail handoff path. None of this evidence is implemented ([DATA_RETENTION.md](DATA_RETENTION.md) § Learning evidence).
