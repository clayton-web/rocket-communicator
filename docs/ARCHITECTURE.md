...
| **Interpretation occurrence**                                          | Shared backend     | `interpretation_runs` / `InterpretationRun` in `@aicaa/db` — completed-occurrence persistence foundation (idempotency + successful outcomes only) (**D161**). Shared application producer wiring is authorized by **D169** (S3.1 first); not yet implemented and still unreachable from Owner HTTP/Android | No “one interpretation forever per source.” Occurrence is not canonical Task truth                                                                  |
...
- **D169** authorizes controlled S3 implementation with Owner manual capture (`sourceKind = owner_manual_capture`) as the first source-neutral producer; Gmail and SMS adapters remain unauthorized until separately approved. S3.1 is backend-only (no Owner HTTP/Android reachability).
