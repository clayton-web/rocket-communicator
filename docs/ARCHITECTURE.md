...
| **Interpretation occurrence**                                          | Shared backend     | `interpretation_runs` / `InterpretationRun` in `@aicaa/db` — completed-occurrence persistence foundation (idempotency + successful outcomes only) (**D161**). Shared application producer wiring is authorized by **D169** and implemented in S3.1. **D170** authorizes S3.2 Owner `POST /api/v1/manual-captures` as the first HTTP adapter, now implemented. **D171** authorizes S3.3 Android capture-to-proposal client reachability, now **implemented**. S3 for Owner manual capture is complete at the locked capture-to-proposal boundary; the later Android proposal-lifecycle UI is implemented under **D176 / S5**. **D179** / **D180** authorize S7 Gmail Review with Rocket; that Gmail surface is **implemented** and **S7 is complete** at the locked Gmail boundary (intake, Review adapter, Android UI, Exclude Sender), not Production-activated. **D181** authorizes SMS/text Review with Rocket as a Messages-specific adapter into that same shared path; device-local Google Messages notification intake, the Messages Review server/persistence boundary, and Android Review invocation are **implemented**, and Exclude Number remains **not implemented** | No “one interpretation forever per source.” Occurrence is not canonical Task truth                                                                  |
...
- Representing `trigger = owner_review` does **not** by itself authorize Owner-review APIs, Review-with-Rocket UI, exclusions, automatic-processing changes, notifications, cron, or Production flags. **D179** is the separate S7 authorization for Gmail Review with Rocket. **D181** is the separate authorization for SMS/text Review with Rocket.
- **D169** authorizes controlled S3 implementation with Owner manual capture (`sourceKind = owner_manual_capture`) as the first source-neutral producer. **D179** authorizes the S7 Gmail-specific adapter into that same shared interpretation capability. **D181** authorizes the SMS/text Review Messages-specific adapter into that same shared interpretation capability; device-local Google Messages notification intake, the Owner-authenticated Messages Review server/persistence boundary, and Android Review invocation are implemented, and Exclude Number remains not implemented. S3.1 is implemented as backend infrastructure with no Production activation: the interpretation application service resolves organization-scoped idempotency before interpreting, calls the shared provider outside any database transaction, and commits the occurrence with its 0..N proposals atomically. Its provider composition is default closed, `peopleHints` and unresolved `deadlineExpression` are not persisted, and the service returns canonical proposals plus occurrence metadata rather than persistence identity. **D170** authorizes S3.2, now implemented: Owner-authenticated `POST /api/v1/manual-captures` plus generated public contracts call that service with server-fixed `owner_manual_capture`. The route is a thin adapter — authentication and organization scope, content type, `Idempotency-Key`, body validation, one service call, canonical DTO mapping — while fingerprinting, idempotency resolution, the provider call, and the occurrence transaction stay in S3.1. Raw capture text is transient on the server and never persisted, echoed, or logged, and the public response carries no interpretation provenance. **D171** authorizes S3.3 — Android switches manual capture onto that route and displays returned proposals read-only, with encrypted device-side pending-capture retry state (max 24 hours) — and that slice is **implemented**. S3 for the Owner manual-capture path is therefore complete at the locked capture-to-proposal boundary. The later Android proposal-lifecycle UI is implemented under **D176 / S5**. **D179** and **D180** authorize S7 Gmail Review with Rocket as a Gmail-specific adapter, not a generic `/interpretations` API. That Gmail surface is **implemented** and **S7 is complete** at the locked Gmail boundary: `GET /api/v1/gmail/intake`, `POST /api/v1/gmail/reviews` with server-fixed `gmail` provenance, Android Gmail intake/Review UI, and Gmail Exclude Sender with Android Undo. S7 is **not** Production-activated. **D181** authorizes SMS/text Review with Rocket as a Messages-specific adapter, not a generic `/interpretations` API and not an A5-style SMS poller: device-local Google Messages notification intake, an Owner-authenticated Messages-specific Review route, canonical `CommunicationEvent` / `TemporaryCommunicationExcerpt` at the explicit Review boundary, server-owned `google_messages` provenance, D161 replay, and Google-Messages-specific Exclude Number. `CommunicationEvent.accountId` may become nullable for non-Gmail sources; no synthetic Google Messages `CommunicationAccount` is authorized. Device-local Google Messages notification intake, `POST /api/v1/messages/reviews` server/persistence, and Android Review invocation are **implemented**; Exclude Number remains **not implemented**. Generic arbitrary-source interpretation APIs and Production enablement remain separately unauthorized.
...
**Android manual capture now runs the AI-first path (D154, D171, D176).** Today: Capture → Save → `POST /api/v1/manual-captures` → shared interpretation → zero or more canonical TaskSuggestions → Android Owner review → Accept / Edit / Dismiss, with IME speech-to-text filling the same field. The product target for manual typed or dictated capture is Owner input → interpretation occurrence → zero or more proposed Tasks → Owner review → accept and choose the responsible person, **Me** or a **Recipient** → canonical Task → follow-through (D161, **D164**); the review-and-accept half of that target is **implemented** by **S5 (D176)**. **D169** authorizes controlled S3 backend machinery for that path (S3.1: shared interpretation application service + canonical persistence). **D170** authorizes S3.2 Owner manual-capture HTTP/API (`POST /api/v1/manual-captures` + generated contracts) calling that service, implemented on the backend. **D171** authorizes S3.3 Android capture-to-proposal integration — switch capture onto that route, freeze the durable retry tuple, display returned proposals, and keep encrypted device-side pending-capture retry state for at most 24 hours — and that slice is **implemented**. S3 for Owner manual capture is therefore **complete** at this locked boundary. **D176 / S5** authorizes and implements Accept with affirmative responsibility selection, narrow wording Edit, and Dismiss; Merge and a standalone proposal inbox remain unauthorized. Evolving the one shared proposal path and the one shared interpretation capability (D157) remains required; do not add a second one. **D179** / **D180** authorize S7 Gmail Review with Rocket into that same path; that Gmail surface is **implemented** and **S7 is complete** at the locked Gmail boundary, not Production-activated. **D181** authorizes SMS/text Review with Rocket into that same path as a separate Messages Owner surface; device-local notification intake is **implemented**, and Review with Rocket remains **not implemented**. The Android direct-create classes (`CaptureTaskUseCase`, `TaskOwnerRepository.createCapturedTask`) remain present but unreachable from the Capture UI, kept for rollback and later cleanup, and the backend `POST /api/v1/tasks` endpoint is unchanged.
...
**Automatic email interpretation via A6 is current implementation infrastructure, not the product commissioning target.** Email interpretation is targeted to become Owner-initiated ("Review with Rocket") with sender exclusion; automatic interpretation of non-excluded mail remains a **future** mode (D156) and, when approved, should preferentially use the shared interpretation/proposal architecture rather than extend A6 (**D163**). The three categories are kept distinct in [WORKFLOWS.md](WORKFLOWS.md) §1a. **D179** / **D180** authorize **S7** as the implementation of that Owner-initiated Gmail target along A5 → Gmail-specific adapter → existing shared interpretation; that Gmail surface is **implemented** and **S7 is complete** at the locked Gmail boundary, not Production-activated. S7 does not change the current A6 operational path or require A6 deletion. **D181** authorizes the parallel Owner-initiated SMS/text target along device-local Google Messages notification intake → Messages-specific adapter → existing shared interpretation. SMS must not copy A5 ingestion, must not use A6. Device-local notification intake, the Review adapter, and Android Review invocation are **implemented**. Exclude Number is **not implemented**.

## Reminder and notification architecture

Authoritative rules: [WORKFLOWS.md](WORKFLOWS.md) §10. Product law: D102–D110, D128–D136, D152, D164, D177, D178. Persistence reference: [packages/db/README.md](../packages/db/README.md).

**Shape.** A Reminder Schedule is **due-date-driven and Task-scoped**, surviving reassignment (D102, D104). Occurrences are **09:00 organization-local** on a local calendar date, computed with timezone-aware local-calendar arithmetic and resolved individually to absolute instants. Fixed 24-hour millisecond arithmetic is **prohibited** (D103). One D105 advance reminder the morning before the due date **when the Owner has enabled it** (D178; default ON); one each morning after while incomplete, capped at **14 successful overdue deliveries per generation**, then `requiresOwnerAttention` (D106). Advance OFF retains the deadline and does not change D106 overdue follow-through. Waiting suspends and is the only pause mechanism (D107); advance OFF is not Waiting. Automated sends are attributed to a **`system`** actor; Owner scheduling changes to the **`owner`** actor (D107).

**D152 is product law:** Task → 0..1 deadline → 0..N Owner-controlled reminders. Owner-controlled reminders independent of deadlines are **authorized**; escalation ladders, Owner CC ladders, silent AI scheduling, and general calendar management as the product's purpose remain prohibited. The current engine rules stay due-date-driven until a separately authorized implementation slice evolves this domain — **which is the domain to evolve, not to replace.** **D177 / D178 / S6** use the existing single D105/D106 reminder model with the D105 advance independently Owner-optional. They do **not** implement D152's 0..N Owner-controlled reminders and do **not** build the D109 planned-occurrence table. Those remain product-authorized and **deliberately deferred beyond S6** — not rejected, abandoned, or superseded. S6 must preserve that reserved extension path and must not make later multiple-reminder expansion unnecessarily difficult.

**Audience is not responsibility, and there is an unresolved delivery seam here (D164).** A8 reminder delivery is Recipient-oriented throughout: eligibility requires an active external assignment, and an occurrence without one is skipped as `no_active_assignment` ([WORKFLOWS.md](WORKFLOWS.md) §10a). Owner-responsible reminder delivery is therefore **not implemented and not claimed anywhere**. **D177 / D178 / S6** authorize canonical due-date and urgency display, including on Owner-responsible and evidence-free Tasks, without changing `no_active_assignment`, occurrence processing, or Owner-audience delivery. Closing the delivery seam needs its own later authorized slice and must **evolve this Task-scoped domain** rather than add a second reminder engine, schedule, or occurrence model. If A8 processing is later enabled without further changes, unassigned or Owner-responsible active schedules can generate repeated `no_active_assignment` skips.
...
Concrete excerpt `purgeAt` always (D082); initial deadline seven days from creation — Gmail `syncedAt + 7 days` (D078), Messages Review `reviewedAt + 7 days` (D181); bounded 30-day workflow safety ceiling (not refreshed for long-lived active Tasks) / terminal + 7 days (D020, D082); multi-proposal sibling entitlement so one sibling does not shorten another’s still-valid hold (D082, D161); Owner-authored manual capture raw input is a separate short-lived review-support class (max 7 days from successful interpretation — D162), not TemporaryCommunicationExcerpt; 30-day completed visibility scrub; immediate audio delete on success. Application retention does not delete Gmail mailbox copies (D031). Details: [DATA_RETENTION.md](DATA_RETENTION.md).

**One retention law, two ways of naming the excerpt (D082).** A6 finds its excerpt through the claimed `CommunicationEvent` it processed; that linkage is A6 processing provenance, remains unique per event, and stays A6-only. Gmail Review and Messages Review proposals instead carry an explicit temporary-excerpt linkage on the suggestion, because one Review legitimately yields 0..N siblings (D161) that each hold their own entitlement — a 0..1-per-event constraint is not their cardinality. Both feed one shared resolver that takes the maximum still-valid sibling entitlement, in the same transaction as the lifecycle mutation. Do not build a second retention system for either Review source.
