...
- **The Owner chooses a date; the server derives everything else.** The request carries only `dueLocalDate`. Occurrence dates and instants, generation, advance disposition, status, delivered count, stop reason, and organization are all derived, and the validator **rejects** rather than ignores a request that supplies one — a silently dropped `generation` would let a client believe it had pinned one. The response omits claim leases, worker identifiers, provider message identifiers, raw failure detail, and row identifiers, so lease mechanics remain changeable without a contract break.
- **Reminder writes do not bump the Task `version`.** `due_local_date` is not part of the Task contract, so nothing an ETag describes changes; bumping it would invalidate a client's Task ETag for a change that client cannot observe. Mutations still require `If-Match` because Owner due-date mutation runs under Task concurrency (D045, D104), and generation changes carry their own compare-and-set on the expected generation, so two concurrent changes cannot both win.
- **Task-lifecycle coupling is not wired yet.** A8.3b stops a schedule on due-date removal only. Waiting suspension and resume, and stopping on completion or dismissal, are deferred: Waiting is reachable by both the Owner and a Recipient capability through the shared `persistCapabilityAction` transaction, and implementing one of D107's lifecycle rules without the others would leave a completed Task holding an `active` schedule. Nothing sends, so this is currently inert — but it is a hard prerequisite for A8.4.
