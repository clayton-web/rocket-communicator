...
| **Proposal / candidate**                                               | Shared backend     | `TaskSuggestion` and the Application Suggestion Engine (D080–D085)                                                                                                                              | No `CandidateTask`, inbox-task persistence, or second proposal store before establishing that `TaskSuggestion` cannot evolve to carry the need |
| **Interpretation**                                                     | Shared backend     | `packages/ai` (D085)                                                                                                                                                                            | No per-source implementation of "what constitutes a task." Source-specific **adapters** may differ; the semantics may not                      |
...
**Interpretation semantics.** The target shape for every source is:

```
source → shared interpretation → zero or more proposals → Owner review → canonical Task
```

Zero proposals is a **successful** interpretation outcome meaning no actionable Task was found — not an error and not a fallback to direct creation. Manual capture, Gmail, SMS, and future post-call capture converge on these semantics.
...
Persisted states: `pending` · `approved` · `dismissed` · `merged`. Terminal states do not transition again. Only the **Owner** may approve, edit, dismiss, or merge; AI and voice create suggestions, never Tasks (D038).

| From                          | To        | Actor | Notes                                                                                                                 |
| ----------------------------- | --------- | ----- | --------------------------------------------------------------------------------------------------------------------- |
| pending                       | approved  | Owner | Creates an **unassigned** Task (D080); excerpt safety ceiling (D082); Recipient handoff is a separate mutation (D090) |
...
**Android capture is currently direct-create, and that is not the target (D154).** Today: Capture → Save → `POST /api/v1/tasks` → confirm only on `201`, with IME speech-to-text filling the same field. The product target for manual typed or dictated capture is AI-first: Owner input → shared interpretation → zero or more proposed Tasks → Owner review → **Keep for me** or **Assign** → canonical Task. When that path is authorized it must **evolve the one shared proposal path and the one shared interpretation capability** (D157), not add a second one. The shipped direct-create path remains valid until then; it is **not** permanent architecture.
...
Concrete excerpt `purgeAt` always (D082); ingest `syncedAt + 7 days` (D078); bounded 30-day workflow safety ceiling (not refreshed for long-lived active Tasks) / terminal + 7 days (D020, D082); 30-day completed visibility scrub; immediate audio delete on success. Application retention does not delete Gmail mailbox copies (D031). Details: [DATA_RETENTION.md](DATA_RETENTION.md).
