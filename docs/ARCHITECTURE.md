...
- **D169** authorizes controlled S3 implementation with Owner manual capture (`sourceKind = owner_manual_capture`) as the first source-neutral producer; Gmail and SMS adapters remain unauthorized until separately approved. S3.1 is implemented and backend-only (no Owner HTTP/Android reachability): the interpretation application service resolves organization-scoped idempotency before interpreting, calls the shared provider outside any database transaction, and commits the occurrence with its 0..N proposals atomically. Its provider composition is default closed, and `peopleHints` and unresolved `deadlineExpression` are not persisted.
...
**Android capture is currently direct-create, and that is not the target (D154).** Today: Capture → Save → `POST /api/v1/tasks` → confirm only on `201`, with IME speech-to-text filling the same field. The product target for manual typed or dictated capture is AI-first: Owner input → interpretation occurrence → zero or more proposed Tasks → Owner review → accept and choose the responsible person, **Me** or a **Recipient** → canonical Task → follow-through (D161, **D164**). **D169** authorizes controlled S3 backend machinery for that path (S3.1 first: shared interpretation application service + canonical persistence, unreachable from product surfaces) and requires evolving the one shared proposal path and the one shared interpretation capability (D157), not adding a second one. The shipped Android direct-create path is a legacy/direct-capture shortcut that will eventually be superseded by S3 proposal review; **D169 does not authorize removing or modifying it**, including in S3.1 — switching Android to interpretation/proposal review is a later controlled slice.
