...
| `ENABLE_REMINDER_DELIVERY`      | When exactly `true`, allows reminder occurrence processing to claim and write. **Set nowhere, including Production**, and gated by [Reminder engine operations](#reminder-engine-operations-a8--not-operational). Even when on, A8.4a reaches only a fake transport. |
...
Applying the A8.4a migration still sends nothing. It makes occurrence processing _representable_; the processing endpoint remains disabled by default, drives a fake transport, and is invoked by no cron job.
...
Even with the flag on, A8.4a sends no email: the only transport that exists is a fake one used by tests, and no processing module can reach a Gmail client — a source guard fails the build if one is imported. Real delivery is A8.4b.
