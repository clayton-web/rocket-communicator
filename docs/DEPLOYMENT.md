...
### A8.7 enablement order (not performed)

Recorded so the sequence is decided before anybody is under pressure to improvise it. **None of these steps has been taken, and taking any of them is A8.7 work requiring its own authorization.**

1. Apply the required A8 migrations in Production.
2. Keep every delivery flag disabled while doing so.
3. Enable `ENABLE_OWNER_EVENT_CAPTURE` for a bounded observation window.
4. Verify expiry observation and event-intent creation from the worker response counts and the audit log.
5. Enable `ENABLE_OWNER_EVENT_DELIVERY` in a controlled window.
6. Verify send-to-self delivery and that the `X-Rocket-Generated` marker excludes those messages from ingestion.
7. Complete the A8.6 Owner visibility requirements.
8. Only then consider enabling `ENABLE_REMINDER_DELIVERY` under D108.
9. Create scheduler jobs only during authorized A8.7 work.
