...
| `ENABLE_OWNER_EVENT_DELIVERY`   | When exactly `true`, allows the Owner notification worker to claim and process intents (D135). **Absent everywhere, including Production, and must stay that way.** With it unset `POST /api/v1/internal/notifications/process` returns a zero-work response before opening a database connection: no scan, no claim, no attempt row, no transport. Enabling it alone still sends nothing, because A8.5b's only transport is a test fake and the route constructs none — the response reports `transportConfigured: false` in every environment. Entirely separate from `ENABLE_OWNER_EVENT_CAPTURE`: capture writes intents, delivery processes them, and neither implies the other. `"1"`, `"TRUE"`, `"yes"`, and whitespace variants all leave it **off** |
...
### Owner notification worker (A8.5b — not operational)
...
**It cannot send, and there are two independent reasons rather than one.** `ENABLE_OWNER_EVENT_DELIVERY` is absent everywhere, and the route injects no transport in any environment — the only implementation is a test fake, and the service treats an absent transport exactly as a disabled flag. Either condition alone produces a zero-work response reporting `deliveryEnabled` and `transportConfigured` separately, so an operator can tell which one is holding it shut. The flag is read **before** the database runtime is loaded, so a disabled invocation opens no connection at all. A source guard fails the build if the worker imports the Gmail adapter, a MIME builder, or reminder delivery policy.

**Counts against `owner_notification_intents` and `owner_notification_attempts` will error in Production** until the A8.5a migration is applied there; A8.5b adds no migration of its own.
