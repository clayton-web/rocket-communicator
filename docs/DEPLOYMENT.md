...
| `ENABLE_GMAIL_REVIEW`           | Enables the Gmail Review application surfaces: `GET /api/v1/gmail/intake`, `POST /api/v1/gmail/reviews`, and the Gmail sender-exclusion routes. Exact `'true'` required. Absent/default and every other value — including `"1"`, `"TRUE"`, `"yes"`, and whitespace variants — remain **off**. Does **not** disable A5 connection, OAuth, or sync infrastructure. Disabled S7 routes answer `404` before authentication. Production near-term expected state is absent/OFF until separately authorized |
