...
### Owner notification worker (A8.5b, A8.5c, A8.5d — not operational)

**A second internal worker exists and is separate from the reminder worker in every respect.** `POST /api/v1/internal/notifications/process`, same `CRON_SECRET` bearer family, same Node.js runtime and sixty-second budget, same twenty-five-item batch and fifteen-second deadline reserve. **No scheduler job invokes it and none may be created**; `vercel.json` is unchanged. The two workers are deliberately not merged: reminder occurrence policy and one-shot Owner event delivery have different retry rules, different terminal states, and different tables, and one endpoint doing both would make a single deadline and a single batch serve two unrelated backlogs.

**Since A8.5c a real Gmail adapter exists, and the flag is the only thing holding it shut.** This is a genuine reduction in the number of independent reasons the worker cannot send — A8.5b had two, and now there is one, so `ENABLE_OWNER_EVENT_DELIVERY` must be treated as live ammunition rather than as a formality. It is **absent everywhere, including Production, and must stay that way**; the response still reports `deliveryEnabled` and `transportConfigured` separately so an operator can see which condition applies. The flag is read **before** the database runtime is loaded, any configuration is read, or any credential is touched, so a disabled invocation opens no connection and constructs nothing. Under a test runner, constructing the real sender without an injected fake throws a dedicated safety error rather than returning something that could reach Gmail. A source guard fails the build if the processing service itself imports a provider, a MIME builder, or reminder delivery policy — the adapter composes at the route, not inside the state machine.

**Enabling delivery would send mail from the connected Gmail account to itself.** The destination is resolved server-side from the intent organization's `CommunicationAccount.emailAddress`; there is no configuration setting that redirects it, and `OWNER_ORGANIZATION_ID`, if set, only asserts agreement and fails closed on mismatch. `NEXT_PUBLIC_APP_URL` must be a valid application origin, since Owner notification links are built from it; if it is missing or invalid the route constructs no transport rather than sending mail with a broken or foreign link.

**The A8.5d capability-expiry sweep is implemented and nothing invokes it.** `runCapabilityExpirySweep` is not called by this route, by any other route, or by any scheduler. Wiring it here would have meant either opening a database connection while `ENABLE_OWNER_EVENT_DELIVERY` is unset — destroying the zero-access guarantee above — or changing the contracted worker response, and both are A8.5e decisions. **Do not describe capability expiry as swept or scheduled.** Today it is observed on the Recipient path, exactly as before A8.5d, with the difference that the observation is now durable, audited, and captured.

**Counts against `owner_notification_intents` and `owner_notification_attempts` will error in Production** until the A8.5a migration is applied there; none of A8.5b, A8.5c, or A8.5d adds a migration of its own.
