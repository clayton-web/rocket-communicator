...
- [ ] **A8 Follow-up Engine:** advance/overdue Recipient reminders derive from an **explicitly Owner-selected** Task due date (D102–D106); no escalation stages and no Owner CC ladders (D099). **D152:** Owner-controlled Task reminders (zero or multiple, independent of whether the Task has a deadline) are product-authorized separately and remain **not implemented** — do not treat A8 due-date rules as a product-law bar on that future capability, and do not claim it already ships
...
- [ ] Deferred A8-slice scope absent from unauthorized work: no preset reminder choices, Owner-created additional reminders, custom-reminder routes or UI, recurrence editor, reminder-time picker, Recipient reminder preferences, or AI-controlled scheduling (D110 sequencing). Owner-created Task reminders are **product-authorized by D152** but remain unimplemented and must not be built inside unauthorized A8 slices.
...
- [ ] Gate 6 evidence shows Production at **`D3` / `F1`**, and the **public custom domain** resolves to that F1 deployment. **Currently unmet — Gate 6 is incomplete and the alias is on the `F0` Bearer deployment `dpl_Cs2TrnDsy1KSB3wipCCUt82Hpf8D`**
