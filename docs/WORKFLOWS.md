...
**Rules for this document.** Do not describe the commissioning target as implemented, do not describe the current A6 implementation as the target or as future architecture, and do not present the future automatic mode as either current behaviour or permanently forbidden. An intake surface, an exclusion list, and Owner-initiated review are **not implemented and not authorized** by D156; the operative A6 compatibility behaviour remains §1 until a separately approved change replaces or retires it. Do **not** treat this section as authorization to delete A6 immediately (**D163**).
...
**Direct typed/dictated create remains on `POST /api/v1/tasks` for web, and as unused Android A9.2 source (D149).** Owner Capture on Android no longer uses that path: **S3.3 (D171)** switched it to `POST /api/v1/manual-captures` → shared interpretation → 0..N read-only proposals. That capture-to-proposal path is **implemented** and creates no canonical Task. The product target remains Owner review → accept and answer **one** question, "who is responsible for this Task?", the **Owner (Me)** or a **Recipient** → canonical Task → follow-through; there is no separate Owner-facing Keep action (D164). Zero proposals is truthful success (D161). Selecting a Recipient uses the existing handoff path (D155). Owner-authored raw input may be retained only for proposal review under D162 — it is not TemporaryCommunicationExcerpt content. The review-and-accept half, including responsibility selection UI, remains **unimplemented** and separately unauthorized; when later authorized it must evolve the existing proposal and interpretation path rather than fork it (D157).
