...
**Observation now, personalization later (D155).** Recording learning evidence is **authorized now** and is no longer deferred to A14: revision 0 as presented to the Owner, append-only Owner edits, the finally accepted revision, explicit Keep or successful Assign acceptance-outcome facts, and the recipient when assignment succeeds. Keep must never be inferred from absence of TaskAssignment. Recorded evidence is **dormant** — it must not alter prompts, personalize the first-pass interpretation, auto-assign, silently modify behaviour, or train the model online. **Personalization stays deferred** and needs its own approved decision. What A14 owns is personalization and rule proposal, not the existence of the evidence. **Inert revision-evidence carrier now exists** (`task_suggestion_revisions` in `@aicaa/db`): authorized and implemented as dormant append-only storage only. No producer writes revisions yet — A6 revision capture, Owner-edit capture, and accepted-revision persistence are **not** authorized by the storage foundation. Absence of revision rows for an existing suggestion means “no revision evidence has been recorded,” not absence of a proposal, and must not fabricate history from the mutable TaskSuggestion head.
