...
**Rules for this document.** Do not describe the commissioning target as implemented, do not describe the current A6 implementation as the target or as future architecture, and do not present the future automatic mode as either current behaviour or permanently forbidden. D156 remains the product target and is not itself an implementation authorization. **D179** authorizes **S7 — Gmail Review with Rocket** as the implementation of that target; S7 is **authorized, not implemented**. The operative A6 compatibility behaviour remains §1 until S7 is implemented and a separately approved change replaces or retires A6. Do **not** treat this section as authorization to delete A6 immediately (**D163**).
