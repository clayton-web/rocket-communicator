...
**Direct typed/dictated create is current implementation, not the target (D154).** Manual capture is intended to be **AI-first**: Owner input → context-free interpretation occurrence → zero, one, or several proposed Tasks → Owner review → accept and answer **one** question, "who is responsible for this Task?", the **Owner (Me)** or a **Recipient** → canonical Task → follow-through; there is no separate Owner-facing Keep action (D164). Zero proposals is truthful success (D161). Selecting a Recipient uses the existing handoff path (D155). Owner-authored raw input may be retained only for proposal review under D162 — it is not TemporaryCommunicationExcerpt content. That AI-first path is **not implemented and not authorized here**, and when it is authorized it must evolve the existing proposal and interpretation path rather than fork it (D157). Until then, the direct create route above is the truthful current behaviour on both web and Android (A9.2 / D149).
