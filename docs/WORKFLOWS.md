...
## 1. Gmail → Communication Event → Task Suggestion _(A5 events; A6 preserved compatibility — production-operational)_
...
### 1a. Email interpretation — three categories that must not be blurred (D156, D163)

| Category                                  | Statement                                                                                                                                                                                                                                                                                                                                                                                                                |
| ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Current implementation infrastructure** | Exactly what §1 above describes and what is **production-operational today**: A5 scheduled Gmail polling into `CommunicationEvent`, plus preserved A6 automatic Suggestion Engine processing (heuristic + LLM extraction) for eligible events (D077, D084, D085). A6 is **compatibility/legacy**, not the product target and not a dependency target for new work (**D163**). This is truthful and is **not** retracted. |
| **Current product commissioning target**  | A Gmail message reaches a Rocket **intake surface**; the **Owner sees** the message; the Owner **manually** selects **"Review with Rocket"**; shared interpretation runs **only then** (0..N proposals). The Owner can **Exclude sender**, and an excluded sender must not be processed by Rocket AI in future. **No permanent inclusion list is required.**                                                             |
| **Future automatic mode**                 | Non-excluded incoming messages **may** be interpreted automatically. This remains a legitimate possible product direction and requires its own approved decision. When approved, it should preferentially use the **shared interpretation / InterpretationRun / 0..N TaskSuggestion** architecture rather than extend A6’s legacy semantics (**D163**).                                                                  |

**Rules for this document.** Do not describe the current A6 implementation as the target or as future architecture, and do not present the future automatic mode as either current behaviour or permanently forbidden. D156 remains the product target and is not itself an implementation authorization. **S7 — Gmail Review with Rocket is complete** at the locked Gmail boundary (D179, D180): Owner-authenticated intake, explicit Owner-initiated Review with Rocket, shared interpretation into 0..N proposals, Android Gmail intake/Review UI, and Gmail Exclude Sender. That Gmail path is **implemented, release-gated OFF, not Production-activated** (`ENABLE_GMAIL_REVIEW` absent; S7 routes answer controlled `404`). `INTERPRETATION_AI_ENABLED` is enabled for Manual Capture and Messages Review; that does not release S7. A Gmail Review proposal establishes its D082 excerpt entitlement in the same transaction that creates it, through the same source-neutral retention law Messages Review uses — see [DATA_RETENTION.md](DATA_RETENTION.md#excerpt-retention-transition-table-d082). Do not treat S7 closeout as completion of SMS Review, OAW, or the full commissioning loop. The operative A6 compatibility behaviour remains §1 until a separately approved change replaces or retires A6. Do **not** treat this section as authorization to delete A6 immediately (**D163**).
...
## 3. Google Messages → interpretation → proposals _(D181 Messages Review = CLOSED / PRODUCTION-PROVEN; law: D160; Exclude Number not implemented)_
...
2. Rocket shows recent eligible one-to-one plain-text Google Messages notification items on a **device-local** recent-review surface (bounded/recent; not a historical archive; Google Messages packages allowlisted; fail closed when unsafe). **Implemented** as an in-memory bounded recent list plus a separate Owner Messages surface. Groups, media-only MMS, attachments, and historical conversations remain unsupported.
3. Owner selects an eligible item.
4. Owner explicitly chooses **"Review with Rocket"**.
5. Android sends the selected occurrence to the Owner-authenticated Messages-specific Review route (`POST /api/v1/messages/reviews`). **Server/persistence and Android invocation implemented and Production-proven.** The server creates/reuses canonical `CommunicationEvent`, persists selected text under existing `TemporaryCommunicationExcerpt` retention law with an initial `purgeAt` of `reviewedAt + 7 days`, and runs **shared interpretation** only then (D157, D181), producing **zero, one, or several** proposals. Each proposal the occurrence commits establishes its own D082 entitlement on that excerpt in the same transaction; zero proposals leave the seven-day deadline untouched. `CommunicationEvent.accountId` is null; no synthetic Google Messages `CommunicationAccount`. A6 must not process these events. Notification arrival remains local-only; Review uses the existing S5 proposal architecture.
6. Existing S5 proposal presentation/lifecycle handles 0..N proposals. Owner decides; only then does a canonical Task exist.
7. Owner may choose **Exclude Number** where a valid canonical E.164 sender exists (D181). **Not implemented.** Excluded numbers leave the local review surface once authoritative exclusion state is known and are refused for new Review; exact committed D161 replay remains valid. Immediate in-memory Undo may remove that exclusion. No number-management settings screen or allow list.
...
**Direct typed/dictated create remains on `POST /api/v1/tasks` for web, and as unused Android A9.2 source (D149).** Owner Capture on Android no longer uses that path: **S3.3 (D171)** switched it to `POST /api/v1/manual-captures` → shared interpretation → 0..N proposals. That capture-to-proposal path is **implemented** and creates no canonical Task. The product target remains Owner review → accept and answer **one** question, "who is responsible for this Task?", the **Owner (Me)** or a **Recipient** → canonical Task → follow-through; there is no separate Owner-facing Keep action (D164). Zero proposals is truthful success (D161). Selecting a Recipient uses the existing handoff path (D155). Owner-authored raw input may be retained only for proposal review under D162 — it is not TemporaryCommunicationExcerpt content. The review-and-accept half, including affirmative Me / Recipient responsibility selection, is **implemented** on Android by **S5 (D176)**. Merge, proposal inbox/list-browsing beyond the capture decision loop, and every other exclusion D176 does not lift remain separately unauthorized. The path must continue to evolve the existing proposal and interpretation path rather than fork it (D157). **D179** separately authorizes **S7** Gmail Review with Rocket into this same shared path; S7 is **implemented, release-gated OFF, not Production-activated**.
...
4. The schedule is **established when the Owner sets a due date** — on Task creation, on suggestion approval, or on a later Task update. Nothing about handoff creates, activates, or advances it (D104, §2). Presence of a due date always remains the D106 overdue input. The D105 advance occurrence is independently Owner-optional (D178) and is **not** implied by the mere presence of a due date.
...
#### Automatic advance reminder (D105, D178)

1. When the Owner has enabled the automatic advance reminder for a Task that has a due date, exactly **one** system-generated advance reminder, at 09:00 organization-local on the calendar day **immediately before** the due date. Default is **ON** for existing schedules and for new establishment without an explicit preference (D178).
2. When Automatic Reminder is **OFF**, the due date is retained and the D105 advance occurrence is **not** armed or sent. OFF is not Waiting, not a schedule stop, and does **not** change D106 overdue follow-through. A due-date **change** preserves the current ON/OFF choice. After due-date **removal**, a later re-establishment defaults ON unless explicitly set OFF in that new scheduling act.
3. If that instant is already past when the schedule is established, record the occurrence as **skipped** with reason **`advance_window_elapsed`**.
4. The **establishment** skip decision is made once and persisted. A later scheduler run must never retroactively reclassify an occurrence that is still deliverable, and must never re-derive the establishment decision from the clock.
5. **Two later events may change it, and only two.** The first: a Waiting period that spanned the occurrence. If the Task is still Waiting when the advance instant arrives, resume records the occurrence as **skipped** with a Waiting-specific reason (`skipped_waiting_elapsed`) rather than sending it late — a suspended reminder is not a deferred one (D107). This is not the scheduler reclassifying from the clock; it is the no-backlog rule applied to the advance occurrence, decided by the lifecycle transition that ends the pause. The occurrence's own local date and instant are preserved as history, and the reason stays distinct from `advance_window_elapsed` so the audit trail can say whether the Owner chose the date too late or Waiting covered the reminder. An advance occurrence already delivered, or already skipped at establishment, keeps its existing reason.
...
6. A schedule established **before** 09:00 on the day before the due date may still send that morning.
7. **The advance reminder may be delivered only during its own organization-local calendar day** — the day before the due date — and never once that day has ended (A8.4b.3). A worker reaching it late on that day still sends: "due tomorrow" is true at 23:55. A worker reaching it the next morning does not, because the statement has become false and the due date has arrived. The boundary is a **calendar-day comparison in the organization's zone**, never a fixed number of hours: the advance day is 23 or 25 hours long in the weeks the clocks shift, so an hour budget would cut into that day or spill past midnight into the due date.
8. **A morning that passed unsent is recorded, not left pending.** The worker claims the occurrence, makes no provider call, and records it as **skipped** with reason **`advance_window_elapsed`** — the same fact as item 3, reached a different way. Leaving it untouched would leave the schedule reporting `scheduled` for a morning that can never happen. The reason stays distinct from `skipped_not_eligible`, which means the Task stopped needing a reminder; this one means the reminder was owed and the system did not deliver it.
9. **No** immediate or retroactive advance reminder is ever sent.
10. A Task created **on** its due date gets no advance reminder.
11. A Task created with a due date **already in the past** gets **no backlog**: schedule only the next future 09:00 overdue occurrence, and record the omitted interval once as a truthful audit entry — never as sends.
...
**Worked example.** Task due **Friday**. When Automatic Reminder is ON: advance reminder **Thursday 09:00**; first overdue **Saturday 09:00**; then each following morning at 09:00 while the Task remains incomplete and eligible. When Automatic Reminder is OFF: no Thursday advance; first overdue is still **Saturday 09:00**, and D106 continues unchanged.
...
| No active external assignment | **Prevents** Recipient delivery; occurrence recorded as skipped `no_active_assignment`; the local day is not consumed |
...
No separate pause, snooze, delay, or alternate-cadence control is introduced (D101, D107). Advance-reminder OFF is not a pause: it does not suspend D106 overdue follow-through (D178). `completed` and `dismissed` remain terminal; A8 introduces **no** reopening behaviour. Should a reopen path ever be added, it does **not** reactivate a terminally stopped schedule — an explicit Owner re-save of the due date is required (D109), so a Task reopened long after its due date cannot immediately deliver a backlog for a date already past.
...
The `reminder.no_active_assignment` row describes the shipped Recipient-oriented producer truthfully (D133); the open D164 seam lives in the reminder engine (§10a), not in this taxonomy, and no row here may be restated as though Owner-audience delivery existed.
...
**Observation now, personalization later (D155).** Recording learning evidence is **authorized now** and is no longer deferred to A14: revision 0 as presented to the Owner, append-only Owner edits, the finally accepted **content** revision, and — as an independent axis — the Owner's **affirmative responsibility selection** (the Owner or a Recipient, including which Recipient). A selection is never inferred from the presence or absence of a TaskAssignment (D164); its boundaries and retention are owned by [DATA_RETENTION.md](DATA_RETENTION.md) § Learning evidence. Recorded evidence is **dormant** — it must not alter prompts, personalize the first-pass interpretation, auto-assign, silently modify behaviour, or train the model online. **Personalization stays deferred** and needs its own approved decision. What A14 owns is personalization and rule proposal, not the existence of the evidence. **`task_suggestion_revisions` exists** in `@aicaa/db` as dormant append-only storage. **Prospective A6 only:** each newly created Gmail-extraction TaskSuggestion records revision 0 (`authorKind = ai`) atomically with create; historical A6 suggestions are not backfilled; duplicate/reclaim paths write no revision. Owner-edit revision capture, accepted-revision persistence, responsibility-selection persistence, and interpretation/manual-capture revision producers are **not** implemented. Absence of revision rows means “no revision evidence has been recorded,” not absence of a proposal, and must not fabricate history from the mutable TaskSuggestion head.
