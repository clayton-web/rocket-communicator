...
**Rules for this document.** Do not describe the commissioning target as fully implemented, do not describe the current A6 implementation as the target or as future architecture, and do not present the future automatic mode as either current behaviour or permanently forbidden. D156 remains the product target and is not itself an implementation authorization. **D179** authorizes **S7 — Gmail Review with Rocket** as the implementation of that target; a bounded server-side Gmail intake-read and Review adapter slice is **implemented**, Android Gmail intake/Review UI is **implemented**, and S7 is **not complete**. A bounded Exclude Sender slice is **implemented** (D180). The operative A6 compatibility behaviour remains §1 until a separately approved change replaces or retires A6. Do **not** treat this section as authorization to delete A6 immediately (**D163**).
