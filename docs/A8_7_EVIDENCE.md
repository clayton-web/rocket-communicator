...
**This file is part record and part template.** Sections **1a** through **1d**, **Gate 4**, and **Gate 5** are completed records performed against Production on 2026-08-04 and 2026-08-05. **Gate 6** is a record of an **authorized but incomplete** attempt: it never became live, and Production remains at flag posture `D3` / `F0`. **Live public Production** is the promoted Bearer F0 successor `dpl_Cs2TrnDsy1KSB3wipCCUt82Hpf8D` / `eb8cabe` — see [DEPLOYMENT.md § Current production state](DEPLOYMENT.md#current-production-state). **Stage 12** has a prepared capture-record template below and remains **unbegun**. **A8.7d / A8.7e and everything after Stage 12 remain unfilled template**, and no part of them has been performed. A8.7a created this file; A8.7b-INCIDENT-1b restructured it around the incident.

> **A8.7b is retired.** Its section is replaced by the incident sections below — the local rehearsal (**1a**), the runbook correction (**1b**), the Production schema repair (**1c**, five migrations, no deployment), and the reminder endpoint hotfix (**1d**). [Gate 4](#gate-4--production-migrations-69) and [Gate 5](#gate-5--deploying-the-queued-a84ba86-code) are complete. **[Gate 6](#gate-6--first-controlled-production-enablement-a87c-capture--f0--f1) is incomplete and never became live.** Context: [MILESTONES.md](MILESTONES.md) incident notice.
...
| 2   | The QG `unvalidated_constraints` term was scoped to the `public` schema, and both the published and scoped readings were recorded                                   | G4.11 expects zero unvalidated constraints **anywhere**, and QG's final subquery carries no schema filter                                                        | **Closed by A8.7b-INCIDENT-1i.** Genuine documentation defect: `pg_constraint` is cluster-wide, and Supabase's managed `realtime` schema carries one expected unvalidated constraint that no migration here controls, so the literal "anywhere" expectation cannot hold on a Supabase-managed database. `unvalidated_public = 0` was the meaningful reading. QG now scopes the term to the `public` schema. **The readings observed during this gate are preserved unchanged above** — `unvalidated_all = 1`, `unvalidated_public = 0` — because they record what Production actually returned on 2026-08-05 |
...
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Operator                                                                                                                                                                                                                                                             | Repository owner (single-operator project)                                                                                                                                                             |
| **Authorization reference** — Gate 5 requires its own; Gate 4's does not carry into it                                                                                                                                                                               | Explicit Owner authorization for Gate 5 execution, 2026-08-05 (session); build-command decision finalized same session                                                                                 |
| Execution window (start / end, ISO with timezone)                                                                                                                                                                                                                    | **2026-08-05T22:24:31Z** / **2026-08-05T22:35:00Z** (2026-08-05T15:24:31-0700 / 2026-08-05T15:35:00-0700)                                                                                              |
| Owner no-use window (opened, closed, by whom)                                                                                                                                                                                                                        | Opened **2026-08-05T22:24:31Z** before build by operator; closed **2026-08-05T22:35:00Z** after smoke and inertness checks                                                                             |
| **Exact deployed commit** (full 40-character SHA)                                                                                                                                                                                                                    | `d369c6d567c595ac0fb91b36744a2afb58717ecb`                                                                                                                                                             |
| Local `git rev-parse HEAD` at the time of authorization                                                                                                                                                                                                              | `d369c6d567c595ac0fb91b36744a2afb58717ecb`                                                                                                                                                             |
| Local `git rev-parse origin/main` (**expect `ee5e82a`, unpushed**)                                                                                                                                                                                                   | `ee5e82a0466fa08086fbd007d4b68342f2c8a6db`                                                                                                                                                             |
| `534959d` is an ancestor of the deployment commit (y/n), and the merge that carried it (**expect `68bedff`**)                                                                                                                                                        | **y** — merge `68bedff` ("Merge remote-tracking branch `origin/hotfix/a8-7b-incident-1d-reminder-etag`")                                                                                               |
| `534959d` is **not** an ancestor of `origin/main` (y/n)                                                                                                                                                                                                              | **y**                                                                                                                                                                                                  |
| Deployment worktree path                                                                                                                                                                                                                                             | `/Users/claytonbeckler/rocket communicator`                                                                                                                                                            |
| Worktree `git status --short` empty (y/n)                                                                                                                                                                                                                            | **y**                                                                                                                                                                                                  |
| Worktree migration-directory count (**expect 14**, matching Production)                                                                                                                                                                                              | **14**                                                                                                                                                                                                 |
| `.vercel/project.json` present and linked (y/n)                                                                                                                                                                                                                      | **y** (`apps/web/.vercel/project.json`)                                                                                                                                                                |
| **Prerequisite** — `pnpm verify` green at the commit, run before the window opened (y/n, when)                                                                                                                                                                       | **y** — at `d369c6d`, before window (2026-08-05)                                                                                                                                                       |
| **Prerequisite** — nine PostgreSQL suites green at the commit, on `postgres:17` (y/n, when, pass counts)                                                                                                                                                             | **y** — at `d369c6d`, before window; **9/9** suites green on `postgres:17`                                                                                                                             |
| **Prerequisite** — production bundle guards green against a real local build (y/n)                                                                                                                                                                                   | **y**                                                                                                                                                                                                  |
| **Prerequisite** — `8588c5d` confirmed redeployable, read-only (y/n)                                                                                                                                                                                                 | **y** — `dpl_7vmnL71Lck7JLeftgsJkYVJ4uw82`, Ready                                                                                                                                                      |
| **D2 baseline** — Q2 migration history (**expect exactly 14**, all finished, none rolled back, `applied_steps_count = 1`)                                                                                                                                            | **14** finished rows — _verified_ G5.6                                                                                                                                                                 |
| **D2 baseline** — Q7 (**expect all four notification and reminder tables present**)                                                                                                                                                                                  | **4/4** present — _verified_ G5.6                                                                                                                                                                      |
| **D2 baseline** — QG positive assertion, `public`-scoped (pass/fail, readings)                                                                                                                                                                                       | **pass** — `2, 5, 1, 1, 2, 0, 0` — _verified_ G5.6                                                                                                                                                     |
| **D2 baseline** — Q1 `tasks` count before (`tasks.count.before`). **Gate 4 deviation 3 exists because this was skipped — do not repeat it**                                                                                                                          | **7** — _verified_ G5.6                                                                                                                                                                                |
| **D2 baseline** — `task_reminder_schedules` count before                                                                                                                                                                                                             | **0** — _verified_ G5.6                                                                                                                                                                                |
| **D2 baseline** — `reminder_delivery_attempts` count before (**expect 0**)                                                                                                                                                                                           | **0** — _verified_ G5.6                                                                                                                                                                                |
| **D2 baseline** — `owner_notification_intents` count before (**expect 0**)                                                                                                                                                                                           | **0** — _verified_ G5.6                                                                                                                                                                                |
| **D2 baseline** — `owner_notification_attempts` count before (**expect 0**)                                                                                                                                                                                          | **0** — _verified_ G5.6                                                                                                                                                                                |
| Flags before the build (**expect all three absent**)                                                                                                                                                                                                                 | **All three absent** — _verified_                                                                                                                                                                      |
| Scheduler state before (both jobs inactive; **no** reminder job; **no** notification job)                                                                                                                                                                            | **Not read from cron-job.org** — see deviation 2                                                                                                                                                       |
| Vercel deployment protection on immutable URLs confirmed enabled, read-only (y/n)                                                                                                                                                                                    | **y** — _verified_ G5.6                                                                                                                                                                                |
| **Previous deployment ID** (**expect `dpl_3oder2T3PuDYdmp8pezy6u7RwPRm`**) and its commit (**expect `534959d`**)                                                                                                                                                     | `dpl_3oder2T3PuDYdmp8pezy6u7RwPRm` / `534959d07715ed1cc14e7ee3468706034f5922fe` — retained                                                                                                             |
| Deployment command used (**expect `vercel deploy --prod --skip-domain --yes`**)                                                                                                                                                                                      | `vercel deploy --prod --skip-domain --yes` from repo root — _verified_                                                                                                                                 |
| **New deployment ID**                                                                                                                                                                                                                                                | `dpl_6cVssNpaZeKPBEVGDynd61AoS9nS`                                                                                                                                                                     |
| Immutable deployment URL (recorded; it exists from creation and is not suppressed by `--skip-domain`)                                                                                                                                                                | `https://rocket-communicator-biq1tikgg-claytons-projects-37065b04.vercel.app`                                                                                                                          |
| **Deployment target** (**expect `production`** — anything else must not be promoted)                                                                                                                                                                                 | **production** — _verified_                                                                                                                                                                            |
| **Deployment state** (**expect READY**) and created-at timestamp                                                                                                                                                                                                     | **READY** — created **2026-08-05T22:24:39Z** (2026-08-05T15:24:39-0700)                                                                                                                                |
| **Commit SHA bound to the deployment**, from deployment metadata (**must equal the authorized commit**)                                                                                                                                                              | `d369c6d567c595ac0fb91b36744a2afb58717ecb` — Vercel CLI deploy JSON carries no `gitSource`/`meta`; bound from authorized clean worktree at deploy time (same class as 1d CLI deploy) — see deviation 4 |
| Build command, from project settings                                                                                                                                                                                                                                 | `cd ../.. && pnpm build:domain && pnpm build:db && pnpm --filter @aicaa/web build`                                                                                                                     |
| Build command matches the Owner's [G5.13](DEPLOYMENT.md#g513-the-build-command-question) decision (y/n, which)                                                                                                                                                       | **y** — Owner accepted the **currently configured** command (not `build:vercel`)                                                                                                                       |
| Node version, from project settings                                                                                                                                                                                                                                  | **24.x**                                                                                                                                                                                               |
| **Migration during build (expect none)** — only `prisma generate` may appear in the build log                                                                                                                                                                        | **None** beyond `prisma generate` — _verified_ build log                                                                                                                                               |
| **Route count** — build log (**expect 51 route lines plus a separate `Proxy (Middleware)` line**) and `routes-manifest.json` (**expect 52**). See [G5.12](DEPLOYMENT.md#g512-the-expected-route-set): the two differ by `/_global-error` and describe the same build | Build log: **51** route lines + **`ƒ Proxy (Middleware)`**; local `routes-manifest.json` at deploy commit: **52** — _verified_                                                                         |
| **Route delta** — `/api/v1/internal/notifications/process` **present** (y/n); nothing removed (y/n)                                                                                                                                                                  | **present y**; **nothing removed y** — _verified_ by name in build log                                                                                                                                 |
| `/attention`, `/tasks/[taskId]`, and `/api/v1/tasks/[taskId]/reminder` present in the route set (y/n)                                                                                                                                                                | **y** — _verified_ build log                                                                                                                                                                           |
| **Environment binding** — all five Production-only variables present, `DATABASE_URL` among them (y/n)                                                                                                                                                                | **y** — five Production-only variables present — _verified_                                                                                                                                            |
| **Flags in the deployment environment** (**expect all three absent**)                                                                                                                                                                                                | **All three absent** — _verified_ Production env list                                                                                                                                                  |
| **Pre-promotion inspection** — every [G5.11](DEPLOYMENT.md#g511-pre-promotion-inspection) row passed (y/n)                                                                                                                                                           | **y** — with deviations 1 and 4 noted; inspection completed before explicit promote                                                                                                                    |
| Preview promotion considered or used (**expect: no, never**)                                                                                                                                                                                                         | **no**                                                                                                                                                                                                 |
| `git push origin main` performed before validation (**expect n**)                                                                                                                                                                                                    | **n**                                                                                                                                                                                                  |
| **Promotion command and result**; production domain resolves to the new deployment (y/n)                                                                                                                                                                             | `vercel promote dpl_6cVssNpaZeKPBEVGDynd61AoS9nS --yes` — **Success**; alias resolves to **dpl_6cVssNpaZeKPBEVGDynd61AoS9nS** — **y**                                                                  |
| Previous deployment retained, not deleted (y/n)                                                                                                                                                                                                                      | **y** — `dpl_3oder2T3PuDYdmp8pezy6u7RwPRm` retained                                                                                                                                                    |
| **Smoke 1** — `GET /api/v1/tasks` unauthenticated (**expect typed 401**)                                                                                                                                                                                             | **PASS** — `UNAUTHORIZED`                                                                                                                                                                              |
| **Smoke 2** — `GET /api/v1/tasks/{taskId}/reminder` unauthenticated (**expect typed 401**)                                                                                                                                                                           | **PASS** — `UNAUTHORIZED`                                                                                                                                                                              |
| **Smoke 3** — `GET /api/v1/session` (**expect 200, `owner`, `axford`**)                                                                                                                                                                                              | **PASS** — 200; `role=owner`; `organizationId=axford`                                                                                                                                                  |
| **Smoke 4** — `GET /api/v1/tasks` (**expect 200, cursor page shape**)                                                                                                                                                                                                | **PASS** — 200; `items` + `nextCursor`                                                                                                                                                                 |
| **Smoke 5** — Owner `/tasks` renders (y/n)                                                                                                                                                                                                                           | **y** — task list renders (7 Tasks)                                                                                                                                                                    |
| **Smoke 6** — Owner `/tasks/{taskId}` renders, notes, outcome, reminder state (y/n)                                                                                                                                                                                  | **y** — `task_h2kA-0PVrShpwVJE`: notes, completion outcome, reminder `no_due_date`                                                                                                                     |
| **Smoke 7** — reminder `GET` on a real Task (**expect 200, `state=no_due_date`, ETag ending `v0`**). This is the 1d regression check                                                                                                                                 | **PASS** — 200; `state=no_due_date`; ETag ends **`v0`**                                                                                                                                                |
| **Smoke 8** — reminder `GET` on an unknown Task (**expect typed `NOT_FOUND`**)                                                                                                                                                                                       | **PASS** — `NOT_FOUND`                                                                                                                                                                                 |
| **Smoke 9** — `/attention` loads and **does not reach its error boundary** (y/n)                                                                                                                                                                                     | **y** — 200; no error boundary                                                                                                                                                                         |
| **Smoke 10** — `/attention` section one renders, empty (y/n)                                                                                                                                                                                                         | **y** — empty section one                                                                                                                                                                              |
| **Smoke 11** — `/attention` section two renders, empty (y/n)                                                                                                                                                                                                         | **y** — empty section two                                                                                                                                                                              |
| **Smoke 12** — `GET /c/{token}` capability page renders, non-mutating (y/n)                                                                                                                                                                                          | **n** — not performed; see deviation 3                                                                                                                                                                 |
| **Inertness** — flags after (**expect all three still absent**)                                                                                                                                                                                                      | **All three absent** — _verified_                                                                                                                                                                      |
| **Inertness** — `owner_notification_intents` count after (**expect 0, unchanged**)                                                                                                                                                                                   | **0** — unchanged                                                                                                                                                                                      |
| **Inertness** — `owner_notification_attempts` count after (**expect 0, unchanged**)                                                                                                                                                                                  | **0** — unchanged                                                                                                                                                                                      |
| **Inertness** — `task_reminder_schedules` count after (**expect unchanged from baseline**)                                                                                                                                                                           | **0** — unchanged                                                                                                                                                                                      |
| **Inertness** — `reminder_delivery_attempts` count after (**expect 0, unchanged**)                                                                                                                                                                                   | **0** — unchanged                                                                                                                                                                                      |
| **Inertness** — `tasks` count after (**expect unchanged from Q1**)                                                                                                                                                                                                   | **7** — unchanged from Q1                                                                                                                                                                              |
| **Inertness** — Q2 migration history after (**expect still exactly 14 rows; Gate 5 applies none**)                                                                                                                                                                   | **14** — unchanged                                                                                                                                                                                     |
| **Inertness** — scheduler state after (**expect unchanged; no job created, resumed, or invoked**)                                                                                                                                                                    | **Unchanged** — cron-job.org not contacted                                                                                                                                                             |
| **Inertness** — `/api/v1/internal/notifications/process` invoked by nothing (y/n)                                                                                                                                                                                    | **y**                                                                                                                                                                                                  |
| **Inertness** — Gmail connected, untouched, no API call (y/n)                                                                                                                                                                                                        | **y**                                                                                                                                                                                                  |
| No reminder created or modified during the window (y/n)                                                                                                                                                                                                              | **y**                                                                                                                                                                                                  |
| **Nothing migrated (y/n)**                                                                                                                                                                                                                                           | **y**                                                                                                                                                                                                  |
| **Nothing pushed (y/n)** — `origin/main` still `ee5e82a`                                                                                                                                                                                                             | **y**                                                                                                                                                                                                  |
| **No environment variable or feature flag set, unset, or edited (y/n)**                                                                                                                                                                                              | **y**                                                                                                                                                                                                  |
| Final state (**expect `D3`, equivalently `F0`**)                                                                                                                                                                                                                     | **`D3` / `F0`** — `dpl_6cVssNpaZeKPBEVGDynd61AoS9nS` at `d369c6d`                                                                                                                                      |
| **Gate 6 not begun (y/n)**                                                                                                                                                                                                                                           | **y**                                                                                                                                                                                                  |
...
| #   | Deviation                                                                                                                                                         | Approved plan said                                                                               | Status                                                                                                                                                                                                                       |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Production team aliases were assigned to the new deployment at creation despite `--skip-domain`; alias served the new deployment before explicit `vercel promote` | `--skip-domain` keeps the production domain on the previous deployment until an explicit promote | **Recorded.** Explicit `vercel promote dpl_6cVssNpaZeKPBEVGDynd61AoS9nS --yes` completed after G5.11; alias already pointed at new deployment                                                                                |
| 2   | cron-job.org scheduler baseline not read directly                                                                                                                 | G5.8 requires read-only confirmation of four job states                                          | **Recorded.** No `CRON_JOB_ORG_API_KEY` available; corroborating evidence only: `gmail_sync_runs` last cron trigger **2026-07-20**                                                                                           |
| 3   | Smoke 12 (valid capability `GET /c/{token}`) not performed                                                                                                        | G5.15 check 12 requires a valid issued link                                                      | **Recorded.** Raw capability bearer not retrievable from Production (hash-only storage; same class of limitation as [P1_5_EVIDENCE.md §6](P1_5_EVIDENCE.md#6-production-evidence-limitation--recipient-capability-workflow)) |
| 4   | Vercel CLI deploy JSON carries no commit `gitSource`/`meta`                                                                                                       | G5.11 requires commit SHA in deployment metadata                                                 | **Recorded.** Bound from authorized clean worktree at deploy time (`d369c6d`); same class as 1d CLI deploy                                                                                                                   |
| 5   | First `vercel deploy` attempt from `apps/web` failed (doubled path)                                                                                               | Deploy from clean worktree at authorized commit                                                  | **Recorded.** Second attempt from repo root succeeded                                                                                                                                                                        |
...
| Condition                                         | Physical state recorded | Decision, and who authorized it |
| ------------------------------------------------- | ----------------------- | ------------------------------- |
| **None.** No G5.17 stop condition halted the gate | Not applicable          | Not applicable                  |
...
**Authorized and partially executed 2026-08-05. Gate 6 is INCOMPLETE, capture never became live, and Production remained at `D3` (`F0`) through the Gate 6 window.** Procedure: [DEPLOYMENT.md § Gate 6](DEPLOYMENT.md#gate-6--first-controlled-production-enablement-a87c-capture--f0--f1). Live operator path: [G6.15 Operator execution checklist](DEPLOYMENT.md#g615-operator-execution-checklist). Gate 6 intended to set `ENABLE_OWNER_EVENT_CAPTURE=true` in Production, redeploy so the value bound, verify capture-only inertness of delivery, and do nothing else — moving Production from **`F0`** to **`F1`**. **Live public Production has since moved to the Bearer F0 successor** `dpl_Cs2TrnDsy1KSB3wipCCUt82Hpf8D` / `eb8cabe` (still `F0`) — see [DEPLOYMENT.md § Current production state](DEPLOYMENT.md#current-production-state).

**What actually happened.** The variable was temporarily created in the Vercel Production environment and a production-target deployment, `dpl_7X5r5ypWbq6ipmWMpver6p99p5Xz`, built successfully and reached **READY** with that value in its immutable environment snapshot. **That deployment received only the two default `.vercel.app` aliases.** The project has `autoAssignCustomDomains=false`, so the public custom domain moves only by an explicit alias assignment, and **that assignment did not occur** — the omitted completion step. Through the Gate 6 window the public custom domain remained on `dpl_6cVssNpaZeKPBEVGDynd61AoS9nS`, whose environment snapshot predates the capture flag. **Capture therefore never became live on the public production site.** The capture variable was subsequently **removed** from the Vercel Production environment under separate authorization. **Gate 6 is incomplete; Stage 12, A8.7d, and A8.7e remain unauthorized and unbegun; and nothing in this record authorizes a production action.** **A later authorized Bearer promotion** (separate from Gate 6) placed the public alias on `dpl_Cs2TrnDsy1KSB3wipCCUt82Hpf8D` / `eb8cabe` while remaining at **`F0`** — see [DEPLOYMENT.md § Current production state](DEPLOYMENT.md#current-production-state).

> **⚠ Do not record Gate 6 in the [Gate 5 capture record](#gate-5--deploying-the-queued-a84ba86-code).** That record belongs to an inert code deploy: its rows require all three flags **absent**. Filling a flag enablement into it would erase the evidence that Gate 5's boundary held. Use the table below.

**What a correct Gate 6 records is a flag-binding deployment.** The migration history must be **unchanged** at fourteen rows. `owner_notification_attempts` must remain **0**. The artifact of the gate is a **new deployment ID** plus the exact capture flag value **and the confirmed public custom-domain alias on that deployment**. A READY production-target build that holds only default `.vercel.app` aliases is not a completed gate — that is exactly how the 2026-08-05 attempt fell short. **A Gate 6 record that reports a second A8 flag set, a notification job created, or a worker invocation is evidence that the wrong procedure was followed.**

### Gate 6 capture record

Filled from the partial Gate 6 execution window. A blank row is an incomplete record, not an implied "nothing to report" — Gate 4 deviation 4 and [G6.7](DEPLOYMENT.md#g67-operator-notes--mistakes-already-paid-for) exist because fields went unfilled. **Rows below describe the production-target build, not the live site**; the live site never served this deployment.

| Field                                                                                                                                             | Value                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Operator                                                                                                                                          | Repository owner (single-operator project)                                                                                                                                                                                                                                                                                                                                                                                             |
| **Authorization reference** — Gate 6 requires its own; Gate 5's does not carry into it                                                            | Explicit Owner authorization for Gate 6 execution, 2026-08-05 (session)                                                                                                                                                                                                                                                                                                                                                                |
| Execution window (start / end, ISO with timezone)                                                                                                 | **2026-08-05** (Pacific). Exact start/end ISO timestamps were not transcribed in the evidence handoff — see deviation 2                                                                                                                                                                                                                                                                                                                |
| Owner no-use window (opened, closed, by whom)                                                                                                     | Observed during the Gate 6 window by the operator; exact open/close ISO timestamps not transcribed — see deviation 2                                                                                                                                                                                                                                                                                                                   |
| **F0 baseline** — previous deployment ID and commit (**expect Gate 5 `dpl_6cVssNpaZeKPBEVGDynd61AoS9nS` / `d369c6d` or documented F0 successor**) | `dpl_6cVssNpaZeKPBEVGDynd61AoS9nS` / `d369c6d567c595ac0fb91b36744a2afb58717ecb` — Gate 5 F0 artifact                                                                                                                                                                                                                                                                                                                                   |
| **F0 baseline** — all three A8 flags (**expect all absent**)                                                                                      | **All three absent** before enablement (Gate 5 closeout / F0)                                                                                                                                                                                                                                                                                                                                                                          |
| **F0 baseline** — Q2 migration history (**expect exactly 14**)                                                                                    | **14** finished rows (post-activation reconfirm; unchanged)                                                                                                                                                                                                                                                                                                                                                                            |
| **F0 baseline** — Q8 four counts (**expect `0, 0, 0, 0`** unless accepted exception recorded)                                                     | **`0, 0, 0, 0`** — `task_reminder_schedules`, `reminder_delivery_attempts`, `owner_notification_intents`, `owner_notification_attempts`                                                                                                                                                                                                                                                                                                |
| **F0 baseline** — Q21 expired capabilities (informational)                                                                                        | **Not transcribed** in the evidence handoff — informational only; not a G6.10 post-activation stop — see deviation 2                                                                                                                                                                                                                                                                                                                   |
| **F0 baseline** — notification job absent (y/n)                                                                                                   | **y** — no notification-processing job exists                                                                                                                                                                                                                                                                                                                                                                                          |
| **F0 baseline** — `/attention` loads, no error boundary (y/n)                                                                                     | **y** — loads; unauthenticated path redirects to Owner sign-in                                                                                                                                                                                                                                                                                                                                                                         |
| `ENABLE_OWNER_EVENT_CAPTURE` set to exact string (**expect `true`**)                                                                              | **`true`** — temporarily created in the Vercel Production environment; **subsequently removed** under separate authorization. It was never live on the public custom domain                                                                                                                                                                                                                                                            |
| `ENABLE_OWNER_EVENT_DELIVERY` after (**expect absent**)                                                                                           | **Absent**                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `ENABLE_REMINDER_DELIVERY` after (**expect absent**)                                                                                              | **Absent**                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Redeploy method (`vercel deploy --prod --skip-domain --yes` / Production redeploy / other — name it)                                              | Production-target binding redeploy of local commit `b2e0848` — exact CLI vs dashboard alternative not separately named in the handoff — see deviation 2                                                                                                                                                                                                                                                                                |
| **New deployment ID**                                                                                                                             | `dpl_7X5r5ypWbq6ipmWMpver6p99p5Xz`                                                                                                                                                                                                                                                                                                                                                                                                     |
| **Deployment target** (**expect `production`**)                                                                                                   | **production** — but **not** alias-holding for the public custom domain                                                                                                                                                                                                                                                                                                                                                                |
| **Deployment state** (**expect READY**)                                                                                                           | **READY** — the build succeeded                                                                                                                                                                                                                                                                                                                                                                                                        |
| Promotion / production alias result                                                                                                               | **Incomplete.** `dpl_7X5r5ypWbq6ipmWMpver6p99p5Xz` received **only the two default `.vercel.app` aliases**. With `autoAssignCustomDomains=false`, the required **explicit custom-domain alias assignment did not occur**, so the public custom domain stayed on `dpl_6cVssNpaZeKPBEVGDynd61AoS9nS` (`d369c6d`, F0) — see deviation 3                                                                                                   |
| Commit bound to the new deployment                                                                                                                | `b2e0848abe20354df79669cd802dbec2575af550` — `docs(a8.7): prepare Gate 6 capture activation`                                                                                                                                                                                                                                                                                                                                           |
| **Verification** — other two flags still absent (y/n)                                                                                             | **y** — `ENABLE_OWNER_EVENT_DELIVERY` absent; `ENABLE_REMINDER_DELIVERY` absent                                                                                                                                                                                                                                                                                                                                                        |
| **Verification** — `/attention` still loads (y/n)                                                                                                 | **y** — loads correctly; redirects to Owner sign-in when unauthenticated                                                                                                                                                                                                                                                                                                                                                               |
| **Verification** — Q2 still 14 rows (y/n)                                                                                                         | **y** — exactly **14** migration rows; all finished; none rolled back; `applied_steps_count = 1`                                                                                                                                                                                                                                                                                                                                       |
| **Verification** — `owner_notification_attempts` (**expect 0**)                                                                                   | **0** (Q8)                                                                                                                                                                                                                                                                                                                                                                                                                             |
| **Verification** — notification job still absent; endpoint not invoked (y/n)                                                                      | **y** — no notification-processing job; Gmail Poll inactive; Suggestion Processing inactive; endpoint not invoked                                                                                                                                                                                                                                                                                                                      |
| **Verification** — no Owner/Recipient notification or reminder email; no Gmail API call (y/n)                                                     | **y** — delivery and reminder flags absent; attempts = 0; no send path exercised                                                                                                                                                                                                                                                                                                                                                       |
| Final state (**expect `F1`**, code still `D3`)                                                                                                    | **`F1` not reached.** At Gate 6 close, live Production remained **`D3` / `F0`** on `dpl_6cVssNpaZeKPBEVGDynd61AoS9nS` (`d369c6d`), all three A8 flags absent. `dpl_7X5r5ypWbq6ipmWMpver6p99p5Xz` (`b2e0848`) is a READY, non-live production-target deployment holding default `.vercel.app` aliases only. **Subsequent (separate) Bearer promotion:** public alias now on `dpl_Cs2TrnDsy1KSB3wipCCUt82Hpf8D` / `eb8cabe` — still `F0` |
| **Gate 6 disposition**                                                                                                                            | **INCOMPLETE** — authorized and partially executed; never live. Omitted completion step: explicit custom-domain alias assignment                                                                                                                                                                                                                                                                                                       |
| **Stage 12 / A8.7d / A8.7e not begun beyond this gate (y/n)**                                                                                     | **y** — all three remain unauthorized and unbegun                                                                                                                                                                                                                                                                                                                                                                                      |
...
| #   | Deviation                                                                                                                                                                                                                                                                    | Approved plan said                                                                                           | Status                                                                                                                                                                                                                                                                                                                                                                         |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | Unauthenticated `GET /api/v1/tasks` was intercepted by **Vercel Deployment Protection** and returned **HTTP 302** to Vercel SSO, so the typed application `401 UNAUTHORIZED` was not observed                                                                                | G6.10 check 3 expects typed `401 UNAUTHORIZED` from the application                                          | **Recorded as a verification deviation only — not an application failure.** Deployment Protection sat in front of the app; the probe never reached application auth. No stop condition under G6.11 for application misbehaviour fired. `/attention` still loaded (redirect to Owner sign-in).                                                                                  |
| 2   | Several capture fields (exact execution-window ISO bounds, Owner no-use window ISO bounds, Q21 reading, redeploy method CLI-vs-dashboard) were performed or known at the boundary but not fully transcribed in the evidence handoff                                          | G6.7 / G6.13 — fill every evidence row in the same window                                                    | **Recorded.** Same discipline class as Gate 4 deviation 4. Load-bearing outcomes (deployment ID, alias set, flag values, Q2, Q8, scheduler inactivity, Ready) are filled from the verified handoff                                                                                                                                                                             |
| 3   | **The gate was not completed.** The production-target deployment reached READY carrying the capture flag, but the **explicit custom-domain alias assignment never occurred**, so the public custom domain stayed on the Gate 5 `F0` deployment and capture never became live | G6.9 / G6.10 — the new deployment must hold the production alias, and `F1` must be verified on the live site | **Open and recorded as the reason Gate 6 is incomplete.** `autoAssignCustomDomains=false` means a READY production-target build receives only its default `.vercel.app` aliases. A first documentation reconciliation wrongly recorded this gate as complete at `F1`; this record supersedes that. **No completion, promotion, or alias action is authorized by recording it** |
| 4   | The capture variable was **subsequently removed** from the Vercel Production environment under separate authorization, after the incomplete gate                                                                                                                             | Gate 6 has no removal step; unsetting is a containment action under G6.12                                    | **Recorded as a separate authorized action, not a Gate 6 step.** Its effect is that the Vercel Production environment now matches the live `F0` deployment                                                                                                                                                                                                                     |

### Stop conditions encountered

Each hard stop in [G6.11](DEPLOYMENT.md#g611-stop-conditions) that fired, the physical state recorded at the time, and the decision taken. **A stop that was worked around rather than decided is itself the finding.**

| Condition                                                                                                                                                                                                                                       | Physical state recorded                                                              | Decision, and who authorized it                                                   |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------- |
| **None fired.** No G6.11 stop condition halted the gate. Deviation 1 is a verification-path limitation, not a G6.11 hard stop. **The gate nevertheless did not complete** — see deviation 3; an omitted completion step is not a stop condition | Live custom domain remained on `dpl_6cVssNpaZeKPBEVGDynd61AoS9nS` at `F0` throughout | Not applicable — no containment was required, because the live site never changed |

### Containment actions taken

**Expect none.** Primary containment is unset capture + redeploy, or Instant Rollback to F0 while it remains one step back ([G6.12](DEPLOYMENT.md#g612-containment-and-rollback-posture)).
...
Confirm each explicitly; none is authorized by Gate 6:

- No migration of any kind. **Confirmed** — Q2 still fourteen rows.
- No setting of `ENABLE_OWNER_EVENT_DELIVERY` or `ENABLE_REMINDER_DELIVERY`. **Confirmed** — both absent.
- No push to `main`. **Confirmed** — `origin/main` reconciliation remains a separate Owner decision.
- No promotion of a preview-target deployment. **Confirmed** — the deployment built was production-target, and it was never aliased to the public custom domain.
- No notification or reminder scheduler job created, resumed, edited, or invoked. **Confirmed**.
- No manual invoke of `/api/v1/internal/notifications/process`. **Confirmed**.
- No Owner or Recipient email, and no Gmail API call for notification or reminder send. **Confirmed**.
- **A8.7d and A8.7e are not begun**, and completing Gate 6 would not make them due. **Confirmed** — and Gate 6 did not complete. Stage 12 also remains separately bounded, unauthorized, and unbegun.
...
> **Authorized and partially executed 2026-08-05 as Gate 6; incomplete and never live.** Canonical evidence record: [Gate 6 capture record](#gate-6--first-controlled-production-enablement-a87c-capture--f0--f1). Stage 11 does not carry a second enablement table.

**Preconditions.** Not applicable here — recorded under Gate 6.

**Execution.** Not applicable here — recorded under Gate 6.

**Verification.** Not applicable here — recorded under Gate 6.

**Stop/go criteria.** Not applicable here — recorded under Gate 6.

**Immediate containment.** Not applicable here — recorded under Gate 6.

**Recovery or rollback.** Not applicable here — recorded under Gate 6.

**Evidence to record.** See the [Gate 6 capture record](#gate-6--first-controlled-production-enablement-a87c-capture--f0--f1), which records the partial attempt and the omitted completion step.

### Stage 12 — Capture-only observation

**Not begun. Prepared only. This section does not authorize execution.** Procedure: [DEPLOYMENT.md § Stage 12](DEPLOYMENT.md#stage-12--capture-only-observation-a87c--f1) (`G12.1`–`G12.13`). Live operator path: [G12.9 Operator execution checklist](DEPLOYMENT.md#g129-operator-execution-checklist). Stage 12 would observe capture-only behaviour at **`D3` / `F1`** and does not enable delivery.

> **⚠ Stage 12's `F1` precondition is not currently met.** Gate 6 is **incomplete** and live Production is **`D3` / `F0`** on the Bearer successor `dpl_Cs2TrnDsy1KSB3wipCCUt82Hpf8D` / `eb8cabe`. Every `F1` row below is a template expectation, not an observation.

> **⚠ Do not record Stage 12 into the [Gate 6 capture record](#gate-6--first-controlled-production-enablement-a87c-capture--f0--f1).** Use the table below.

**What a correct Stage 12 records is an observation window, not a flag change.** Capture stays `true`. Delivery and reminder stay **absent**. `owner_notification_attempts` stays **0**. **A Stage 12 record that reports a second A8 flag set, a notification job created, a worker invocation, or any attempt row is evidence that the wrong procedure was followed.**

#### Stage 12 capture record

Fill every row. A blank load-bearing row is an incomplete record.

| Field                                                                                                                                                                        | Value |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- |
| Operator                                                                                                                                                                     |       |
| **Authorization reference** — Stage 12 requires its own; Gate 6's does not carry into it                                                                                     |       |
| Authorized window duration or completion trigger                                                                                                                             |       |
| Window open (ISO with timezone)                                                                                                                                              |       |
| Window close (ISO with timezone)                                                                                                                                             |       |
| **F1 baseline** — deployment ID and commit for the deployment **holding the public custom domain** (must be an `F1` deployment; record what the domain actually resolves to) |       |
| **F1 baseline** — `ENABLE_OWNER_EVENT_CAPTURE` (**expect `true`**)                                                                                                           |       |
| **F1 baseline** — `ENABLE_OWNER_EVENT_DELIVERY` (**expect absent**)                                                                                                          |       |
| **F1 baseline** — `ENABLE_REMINDER_DELIVERY` (**expect absent**)                                                                                                             |       |
| **F1 baseline** — notification job absent (y/n)                                                                                                                              |       |
| **F1 baseline** — Gmail Poll / Suggestion Processing inactive as found (y/n)                                                                                                 |       |
| **F1 baseline** — Q8 four counts at open                                                                                                                                     |       |
| **F1 baseline** — Q15 buckets at open                                                                                                                                        |       |
| Observation mode (**expect: passive / normal Owner and Recipient activity**; no deliberate canary)                                                                           |       |
| Endpoint never invoked (y/n)                                                                                                                                                 |       |
| Notification job never created (y/n)                                                                                                                                         |       |
| Final Q8 four counts (**attempts expect 0**)                                                                                                                                 |       |
| Final Q15 buckets                                                                                                                                                            |       |
| Final state (**expect `D3` / `F1`**, capture only)                                                                                                                           |       |
| **A8.7d / A8.7e not begun (y/n)**                                                                                                                                            |       |

#### Observation log

| Time (ISO) | Q15 `under_1h` | Q15 `h1_to_24` | Q15 `over_24h` | Q8 schedules | Q8 reminder attempts (**0**) | Q8 intents | Q8 notification attempts (**0**) | Event types observed |
| ---------- | -------------- | -------------- | -------------- | ------------ | ---------------------------- | ---------- | -------------------------------- | -------------------- |
|            |                |                |                |              |                              |            |                                  |                      |
...
**Expect none.** Primary containment: unset capture + redeploy ([G12.11](DEPLOYMENT.md#g1211-containment-and-rollback)).
...
Confirm each explicitly; none is authorized by Stage 12:

- No setting of `ENABLE_OWNER_EVENT_DELIVERY` or `ENABLE_REMINDER_DELIVERY`.
- No notification or reminder scheduler job created, resumed, edited, or invoked.
- No manual invoke of `/api/v1/internal/notifications/process`.
- No deliberate single-event canary (Stage 14 / A8.7d).
- No deploy, promote, or push to `main`.
- **A8.7d and A8.7e are not begun**, and completing Stage 12 does not make them due.
