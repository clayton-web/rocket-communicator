...
**This file is part record and part template.** Sections **1a** through **1d**, **Gate 4**, and **Gate 5** are completed records performed against Production on 2026-08-04 and 2026-08-05. **Gate 6 / A8.7c and everything after Gate 5 is unfilled template**, and no part of it has been performed. Gate 6 preparation added the capture-record template below; it remains unbegun. A8.7a created this file; A8.7b-INCIDENT-1b restructured it around the incident.

> **A8.7b is retired.** Its section is replaced by the incident sections below — the local rehearsal (**1a**), the runbook correction (**1b**), the Production schema repair (**1c**, five migrations, no deployment), and the reminder endpoint hotfix (**1d**). [Gate 4](#gate-4--production-migrations-69) and [Gate 5](#gate-5--deploying-the-queued-a84ba86-code) are complete. **[Gate 6](#gate-6--first-controlled-production-enablement-a87c-capture--f0--f1) is prepared and unbegun.** Context: [MILESTONES.md](MILESTONES.md) incident notice.
...
**Not begun. Prepared only. This section does not authorize execution.** Procedure: [DEPLOYMENT.md § Gate 6](DEPLOYMENT.md#gate-6--first-controlled-production-enablement-a87c-capture--f0--f1). Live operator path: [G6.15 Operator execution checklist](DEPLOYMENT.md#g615-operator-execution-checklist). Gate 6 sets `ENABLE_OWNER_EVENT_CAPTURE=true` in Production, redeploys so the value binds, verifies capture-only inertness of delivery, and does nothing else. It moves Production from **`F0`** to **`F1`**.
...
**What a correct Gate 6 records is a flag-binding deployment.** The migration history must be **unchanged** at fourteen rows. `owner_notification_attempts` must remain **0**. The artifact of the gate is a **new deployment ID** plus the exact capture flag value. **A Gate 6 record that reports a second A8 flag set, a notification job created, or a worker invocation is evidence that the wrong procedure was followed.**

### Gate 6 capture record

Fill every row. A blank row is an incomplete record, not an implied "nothing to report" — Gate 4 deviation 4 and [G6.7](DEPLOYMENT.md#g67-operator-notes--mistakes-already-paid-for) exist because fields went unfilled.

| Field                                                                                                                                                                                                                                                                | Value |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- |
| Operator                                                                                                                                                                                                                                                             |       |
| **Authorization reference** — Gate 6 requires its own; Gate 5's does not carry into it                                                                                                                                                                               |       |
| Execution window (start / end, ISO with timezone)                                                                                                                                                                                                                    |       |
| Owner no-use window (opened, closed, by whom)                                                                                                                                                                                                                        |       |
| **F0 baseline** — previous deployment ID and commit (**expect Gate 5 `dpl_6cVssNpaZeKPBEVGDynd61AoS9nS` / `d369c6d` or documented F0 successor**)                                                                                                                    |       |
| **F0 baseline** — all three A8 flags (**expect all absent**)                                                                                                                                                                                                         |       |
| **F0 baseline** — Q2 migration history (**expect exactly 14**)                                                                                                                                                                                                       |       |
| **F0 baseline** — Q8 four counts (**expect `0, 0, 0, 0`** unless accepted exception recorded)                                                                                                                                                                        |       |
| **F0 baseline** — Q21 expired capabilities (informational)                                                                                                                                                                                                           |       |
| **F0 baseline** — notification job absent (y/n)                                                                                                                                                                                                                      |       |
| **F0 baseline** — `/attention` loads, no error boundary (y/n)                                                                                                                                                                                                        |       |
| `ENABLE_OWNER_EVENT_CAPTURE` set to exact string (**expect `true`**)                                                                                                                                                                                                 |       |
| `ENABLE_OWNER_EVENT_DELIVERY` after (**expect absent**)                                                                                                                                                                                                              |       |
| `ENABLE_REMINDER_DELIVERY` after (**expect absent**)                                                                                                                                                                                                                 |       |
| Redeploy method (`vercel deploy --prod --skip-domain --yes` / Production redeploy / other — name it)                                                                                                                                                                 |       |
| **New deployment ID**                                                                                                                                                                                                                                                |       |
| **Deployment target** (**expect `production`**)                                                                                                                                                                                                                      |       |
| **Deployment state** (**expect READY**)                                                                                                                                                                                                                              |       |
| Promotion / production alias result                                                                                                                                                                                                                                  |       |
| Commit bound to the new deployment                                                                                                                                                                                                                                   |       |
| **Verification** — other two flags still absent (y/n)                                                                                                                                                                                                                |       |
| **Verification** — `/attention` still loads (y/n)                                                                                                                                                                                                                    |       |
| **Verification** — Q2 still 14 rows (y/n)                                                                                                                                                                                                                            |       |
| **Verification** — `owner_notification_attempts` (**expect 0**)                                                                                                                                                                                                      |       |
| **Verification** — notification job still absent; endpoint not invoked (y/n)                                                                                                                                                                                         |       |
| **Verification** — no Owner/Recipient notification or reminder email; no Gmail API call (y/n)                                                                                                                                                                        |       |
| Final state (**expect `F1`**, code still `D3`)                                                                                                                                                                                                                       |       |
| **Stage 12 / A8.7d / A8.7e not begun beyond this gate (y/n)**                                                                                                                                                                                                        |       |
...
- No push to `main`.
- No promotion of a preview-target deployment.
...
- No Owner or Recipient email, and no Gmail API call for notification or reminder send.
- **A8.7d and A8.7e are not begun**, and completing Gate 6 does not make them due.
...
> **Canonical evidence record:** [Gate 6 capture record](#gate-6--first-controlled-production-enablement-a87c-capture--f0--f1). Stage 11 does not carry a second enablement table.

**Preconditions.** Not applicable here — record under Gate 6.

**Execution.** Not applicable here — record under Gate 6.

**Verification.** Not applicable here — record under Gate 6.

**Stop/go criteria.** Not applicable here — record under Gate 6.

**Immediate containment.** Not applicable here — record under Gate 6.

**Recovery or rollback.** Not applicable here — record under Gate 6.

**Evidence to record.** Fill the [Gate 6 capture record](#gate-6--first-controlled-production-enablement-a87c-capture--f0--f1) only.
