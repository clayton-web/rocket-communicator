...
Task reads expose additive canonical `dueLocalDate` and read-time `derivedUrgency` (S6.1 / D177). Instant-typed `dueAt` remains compatibility debt and is not the source of those labels. The local date is never reconstructed from `dueAt`.
...
**Status: implemented in `apps/web`, reached by Android Capture (S3.3), and Production-active.** S3 for Owner manual capture is **complete** at this locked capture-to-proposal boundary. Handlers exist and are contract-tested. Interpretation stays default closed without the flag: without `INTERPRETATION_AI_ENABLED` the route answers `503 DEPENDENCY_UNAVAILABLE`. **In Production that flag is enabled**; Manual Capture uses shared interpretation. Gmail Review cannot use it while `ENABLE_GMAIL_REVIEW` keeps S7 unavailable. `operationId`: `createManualCapture`. **D171** authorizes the Android Owner client to call this route for S3.3 capture-to-proposal, and that client is **implemented**: Android manual capture posts here and renders the returned proposals. The Android proposal-lifecycle UI authorized by **D176 / S5** is **implemented** (Accept with affirmative Me / saved-Recipient responsibility, narrow wording Edit, and Dismiss). Merge, a standalone proposal inbox, and every other exclusion D176 does not lift remain separately unauthorized.

| Method | Path                      | Purpose                                                                   |
| ------ | ------------------------- | ------------------------------------------------------------------------- |
| POST   | `/api/v1/manual-captures` | Interpret Owner capture text into 0..N proposals (S3.2; D169, D170, D171) |

This is the **implemented** Owner-initiated **manual-capture** interpretation HTTP surface. It is **not** a generic `/interpretations` endpoint: the server fixes provenance to `owner_manual_capture`; clients do not choose Gmail/SMS source, and no such field is accepted. **D179 / S7** authorizes a Gmail-specific adapter into the same shared interpretation service (`POST /api/v1/gmail/reviews`, **implemented**; S7 complete at the Gmail boundary and not Production-activated) and does **not** authorize a generic `/interpretations` API. Organization scope comes from trusted Owner authentication — never from the body. Capability/Recipient auth is invalid for this route. Required `Idempotency-Key` reuses the existing parameter contract.
...
**Request surface is `dueLocalDate` plus optional `advanceEnabled`.** `SetTaskReminderRequest` requires `dueLocalDate`, a canonical `YYYY-MM-DD` organization-local calendar date, and may include `advanceEnabled` for the existing D105 Automatic Reminder (D178). Omitting `advanceEnabled` on a new establishment or after due-date removal defaults ON; omitting it on a live schedule preserves the persisted ON/OFF value. There is no reminder-time field (09:00 is a constant, D103), no preset interval (retired, D102), no recurrence, and no timezone field — the scheduling timezone is derived from organization configuration, not chosen by the Owner. The validator is an **allowlist**: any other top-level property, any nested object, and every server-derived field is rejected with `400 VALIDATION_ERROR`. Unknown fields are **not** ignored, matching the schema's `additionalProperties: false`. Duplicate JSON keys follow standard last-wins parsing and need no special handling.

**Response is a read-only projection.** `TaskReminderState` returns `taskId`, `etag`, `dueLocalDate`, `schedulingTimeZone`, `state`, `generation`, `advance`, `nextOverdueOccurrence`, `overdueDeliveredCount`, `requiresOwnerAttention`, `stopReason`, and `advanceEnabled`. `state` distinguishes `no_due_date`, `not_scheduled`, `active`, `suspended_waiting`, and `stopped`, and is authoritative for whether anything will be sent; `advance` and `nextOverdueOccurrence` are the generation's recorded decisions and become history once `state` is `stopped`. `advanceEnabled` is the Owner D178 preference (`true` ON, `false` OFF, `null` when no schedule exists). Local calendar dates are canonical `YYYY-MM-DD` text and instants are ISO-8601 UTC; the local date is never reconstructed from `dueAt`.
...
| GET    | `/api/v1/gmail/intake`                          | Eligible Gmail intake items for Review with Rocket                           | Implemented; release-gated `404` while `ENABLE_GMAIL_REVIEW` is off (S7 / D179, D180) |
| POST   | `/api/v1/gmail/reviews`                         | Review one eligible Gmail occurrence with Rocket                             | Implemented; release-gated `404` while `ENABLE_GMAIL_REVIEW` is off (S7 / D179, D180) |
| POST   | `/api/v1/gmail/sender-exclusions`               | Exclude a Gmail sender from Review with Rocket                               | Implemented; release-gated `404` while `ENABLE_GMAIL_REVIEW` is off (S7 / D180)       |
| DELETE | `/api/v1/gmail/sender-exclusions/{exclusionId}` | Remove a Gmail sender exclusion                                              | Implemented; release-gated `404` while `ENABLE_GMAIL_REVIEW` is off (S7 / D180)       |
...
Public Gmail DTOs never include refresh/access tokens, ciphertext, encryption key versions, OAuth codes, or PKCE secrets. Internal poll uses `InternalCronBearer` (configured `CRON_SECRET`), not Owner session and not public unauthenticated access. The application owns the Application Polling Engine; the scheduler is external (D079). GET on the internal Gmail poll route is a **secret-authenticated scheduler exception** for hosts whose schedulers prefer GET (e.g. Vercel Cron)—do not copy this pattern to public Recipient routes (D050). Preferred initial production adapter is HTTP **POST** from **cron-job.org** (or any compatible External Scheduler) every five minutes. External Scheduler invocations never initialize History cursors; Owner manual sync must seed first. A5 does **not** expose a general-purpose communication-event archive or browser (D073). **D179** authorizes the minimum Owner-authenticated Gmail intake read surface required for S7 Review with Rocket; that surface is **implemented** as `GET /api/v1/gmail/intake` (eligible Gmail only; no excerpt bodies; not a generic CommunicationEvent browser) together with `POST /api/v1/gmail/reviews` (Gmail-specific adapter into the shared interpretation service). **D180** implements Gmail-specific Exclude Sender. **S7 is complete** at the locked Gmail boundary and is **not** Production-activated: while `ENABLE_GMAIL_REVIEW` is absent, the four S7 routes answer `404` before authentication. Shared interpretation is already Production-active for authorized KEEP-LIVE surfaces (Manual Capture, Messages Review); this gate does not change those routes or A5 connection/OAuth/sync.

### Owner Gmail intake and Review with Rocket (S7 / D179)

**Status: S7 complete at the locked Gmail boundary; implemented in `apps/web` and Android Gmail intake/Review; not activated in Production.** Handlers exist and are contract-tested. While `ENABLE_GMAIL_REVIEW` is not exactly `"true"`, both routes answer `404 NOT_FOUND` (`This application surface is unavailable.`) with `Cache-Control: no-store` before authentication, database runtime, or interpretation. When the release gate is on, a missing interpretation provider still answers `503 DEPENDENCY_UNAVAILABLE`. `operationId`s: `listGmailIntake`, `createGmailReview`. Android Gmail intake/Review UI is **implemented**.

`GET /api/v1/gmail/intake` is Owner-authenticated and organization-scoped. The contracted response set includes `404` for release-gate unavailability. It returns currently reviewable Gmail occurrences already stored by A5: active Inbox-eligible events (D068) that still have an unpurged, unexpired `TemporaryCommunicationExcerpt` and whose normalized sender is not excluded for this organization (D180). Cursor-paginated newest first (`receivedAt` DESC, then `id` DESC). The listing scans a bounded window of active Gmail candidates and filters Inbox eligibility and sender exclusions in memory; when that scan budget is reached before the keyset is exhausted, `nextCursor` continues from the last scanned candidate even if the page contains no eligible items, so older eligible mail behind a run of ineligible candidates remains reachable. Response items are deliberately narrow: `id` (CommunicationEvent id for a later Review), `fromAddress`, `subject`, `snippet`, `receivedAt`. Excerpt bodies, labels, recipients, attachments, A6 processing state, provider message ids, and organization/account ids are omitted.

`POST /api/v1/gmail/reviews` is the Gmail-specific Owner-authenticated adapter. Request body (`CreateGmailReviewRequest`, `additionalProperties: false`) is only `communicationEventId`. Clients do not send `sourceKind`, `organizationId`, `rawInput`, or `capturedAt`. The server resolves the A5 event, verifies organization ownership, reads the temporary capped excerpt, and calls the shared interpretation service with server-fixed `sourceKind = gmail`. Required `Idempotency-Key` reuses the existing parameter contract. Success is HTTP **200** with `{ idempotentReplay, interpretedAt, taskSuggestions }` — the same public interpretation result shape as manual capture. Exact S3.1 / D161 replay semantics apply: same `(organizationId, Idempotency-Key)` + same reconstructed fingerprint replays the committed result; same key + different fingerprint is `409 IDEMPOTENCY_KEY_CONFLICT`. Current Inbox eligibility is required to **start a new** Gmail interpretation. An excluded sender likewise refuses a **new** interpretation (`409 DOMAIN_CONFLICT`). An exact durable replay may return the already-persisted result (`idempotentReplay: true`) without calling the provider again even if the source is no longer currently Inbox-eligible or the sender was later excluded, provided the temporary excerpt still exists so the fingerprint can be reconstructed. Unavailable or purged excerpts remain fail-closed (`409 DOMAIN_CONFLICT`) and are not generalized into fingerprint-free / key-only replay — that response is the same whether or not a prior run exists, because the fingerprint cannot be reconstructed. Missing/other-org events are `404 NOT_FOUND`; a first request against a currently ineligible event is `409 DOMAIN_CONFLICT`. The route creates no canonical Task, approves nothing, records no responsibility selection, and writes no TaskAssignment. Resulting suggestions remain on the existing S5 proposal lifecycle.

### Owner Gmail Exclude Sender (S7 / D180)

**Status: S7 complete at the locked Gmail boundary; implemented in `apps/web` and Android Gmail intake; not activated in Production.** While `ENABLE_GMAIL_REVIEW` is not exactly `"true"`, both routes answer `404` before authentication. `operationId`s: `createGmailSenderExclusion`, `deleteGmailSenderExclusion`.

`POST /api/v1/gmail/sender-exclusions` is Owner-authenticated. Request body (`CreateGmailSenderExclusionRequest`, `additionalProperties: false`) is only `communicationEventId`. The server resolves the A5 Gmail event and persists an organization-scoped exclusion keyed by the existing Gmail-ingestion normalized From address. Re-excluding the same sender is idempotent. The unparseable-From sentinel is refused (`400 VALIDATION_ERROR`). The public response is `{ id, createdAt }` and does not echo the excluded address. `DELETE /api/v1/gmail/sender-exclusions/{exclusionId}` removes that organization's exclusion. Other-organization ids are `404 NOT_FOUND`. This is not a generic `/communications/exclusions` API, not a sender-management list, and not an allow list. A5 ingestion is unchanged.

### Owner Google Messages Review with Rocket (D181)

**Status: D181 Messages Review = CLOSED / PRODUCTION-PROVEN. Exclude Number is not implemented.** Handler exists and is contract-tested. Android posts `sourceOccurrenceId`, `selectedText`, and `observedAt` with `Idempotency-Key` only after an explicit Owner Review with Rocket. Interpretation stays default closed without the flag: without `INTERPRETATION_AI_ENABLED` the Review route answers `503 DEPENDENCY_UNAVAILABLE`. **In Production that flag is enabled**; Messages Review uses shared interpretation. `operationId`: `createMessagesReview`.

`POST /api/v1/messages/reviews` is the Messages-specific Owner-authenticated adapter. Request body (`CreateMessagesReviewRequest`, `additionalProperties: false`) is `sourceOccurrenceId`, `selectedText`, and `observedAt`. Clients do not send `sourceKind`, `organizationId`, `rawInput`, `capturedAt`, sender, phone number, or conversation title. The server creates or reuses a canonical `CommunicationEvent` with `sourceType = google_messages` and a null `accountId` (no synthetic CommunicationAccount), persists selected text only on `TemporaryCommunicationExcerpt`, and calls the shared interpretation service with server-fixed `sourceKind = google_messages`. Required `Idempotency-Key` reuses the existing parameter contract. Success is HTTP **200** with `{ idempotentReplay, interpretedAt, taskSuggestions }` — the same public interpretation result shape as Gmail Review and manual capture. Exact S3.1 / D161 replay semantics apply: same `(organizationId, Idempotency-Key)` + same occurrence/fingerprint replays the committed result; same key + different occurrence/fingerprint is `409 IDEMPOTENCY_KEY_CONFLICT`. The route creates no canonical Task, approves nothing, records no responsibility selection, and writes no TaskAssignment. Resulting suggestions remain on the existing S5 proposal lifecycle. This is not a generic `/interpretations` API, not A6, and not an A5-style SMS poller.
