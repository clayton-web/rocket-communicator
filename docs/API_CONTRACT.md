...
This is the sole **implemented** Owner-initiated interpretation HTTP surface. It is **not** a generic `/interpretations` endpoint: the server fixes provenance to `owner_manual_capture`; clients do not choose Gmail/SMS source, and no such field is accepted. **D179 / S7** authorizes a later Gmail-specific adapter into the same shared interpretation service and does **not** authorize a generic `/interpretations` API. Organization scope comes from trusted Owner authentication — never from the body. Capability/Recipient auth is invalid for this route. Required `Idempotency-Key` reuses the existing parameter contract.
