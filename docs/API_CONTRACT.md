...
| GET    | `/api/v1/gmail/intake`                          | Eligible Gmail intake items for Review with Rocket                           | Implemented (S7 backend slice; D179, D180) |
| POST   | `/api/v1/gmail/reviews`                         | Review one eligible Gmail occurrence with Rocket                             | Implemented (S7 backend slice; D179, D180) |
...
**Status: implemented in `apps/web` and Android Gmail intake; not complete S7; not activated in Production.** `operationId`s: `createGmailSenderExclusion`, `deleteGmailSenderExclusion`.
