...
### Internal reminder processing (A8.4a foundation, A8.4b.1 real overdue transport — contracted, disabled, never deployed)
...
**Only overdue occurrences are processed.** Advance reminders have persistence and terminalization support but no scan predicate, so no worker path claims one, and the reminder transport refuses an `advance` occurrence outright. Delivering them is A8.4b.3. The D129 stopping rule for repeated ambiguity is **not enforced** here; A8.4b.1 records terminal ambiguous outcomes durably so A8.4b.2 can derive the count, and adding `repeated_ambiguous_outcomes` to `TaskReminderStopReason` will be that slice's contract change.
