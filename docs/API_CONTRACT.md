...
**`POST /api/v1/tasks` is current wire behaviour, not permanent capture UX (D154).** The shipped direct-create path (web and Android A9.2) remains the truthful interim/legacy shortcut. The product target is AI-first interpretation → proposal review → Owner decision → canonical Task. **D169** authorizes controlled S3 backend machinery for that target (S3.1 first) but does **not** authorize changing, removing, or replacing this endpoint. **D170** authorizes a separate Owner manual-capture route `POST /api/v1/manual-captures` (S3.2) — **authorized, not yet implemented** — without replacing this direct-create endpoint. Creating an unassigned Task through this endpoint records no responsibility selection and is not evidence of one (**D164**, D155).

### Owner manual capture (S3.2 / D170) — authorized, not yet implemented

**Status: authorized by D170; not implemented.** When implemented, this is the sole Owner-initiated interpretation HTTP surface authorized for S3.2. It is **not** a generic `/interpretations` endpoint: the server fixes provenance to `owner_manual_capture`; clients do not choose Gmail/SMS source. Organization scope comes from trusted Owner authentication — not from the body. Capability/Recipient auth is invalid for this route. Required `Idempotency-Key` reuses the existing parameter contract. Request body fields are only `rawInput` (max **4000** characters), required `capturedAt` (ISO-8601 with explicit offset; client-owned; not server clock), and optional `timezone`. Success is HTTP **200** with `{ idempotentReplay, interpretedAt, taskSuggestions }` reusing the canonical public `TaskSuggestion` schema — including truthful zero-proposal `taskSuggestions: []`. Exact S3.1 replay semantics apply at the HTTP boundary. Public responses must not expose `interpretationRunId`, fingerprints, persisted idempotency keys, model/policy versions, or raw-input echo. Raw capture text is transient and not retained. Adding the route does not authorize Production activation or enabling `INTERPRETATION_AI_ENABLED`. Android capture switching and proposal UI remain unauthorized.
