...
| Task Reminder Schedule read shape             | Absent                           | **Add** as **read-only** schedule status for the Owner surface: due date, timezone, generation, status, advance disposition, next overdue occurrence, per-generation delivered count, `requiresOwnerAttention` (D104, D106). **No** Phase 1 preset field — presets are retired (D102). **No** reminder-time field — 09:00 is a constant (D103) |
...
| Owner due-date mutation                       | Present (task update / approve)  | **Align** so setting, changing, or removing the due date creates, recalculates, or stops the Task-scoped schedule under existing `If-Match` concurrency (D045, D104). A same-value save must not open a generation                                                                                                                             |
