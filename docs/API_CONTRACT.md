...
**Status: server/persistence Review boundary and Android Review invocation implemented; Exclude Number and Production activation are not.** Handler exists and is contract-tested. Android posts `sourceOccurrenceId`, `selectedText`, and `observedAt` with `Idempotency-Key` only after an explicit Owner Review with Rocket. Interpretation stays default closed: without `INTERPRETATION_AI_ENABLED` the Review route answers `503 DEPENDENCY_UNAVAILABLE`. `operationId`: `createMessagesReview`.
