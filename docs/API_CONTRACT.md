...
**Only overdue occurrences are processed.** Advance reminders have persistence and terminalization support but no scan predicate, so no worker path claims one, and the reminder transport refuses an `advance` occurrence outright. Delivering them is A8.4b.3.
