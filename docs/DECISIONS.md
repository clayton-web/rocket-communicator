...
**Notes:** Owner decision 2026-08-11 (documentation/governance only). Settles the persistence representation left deliberately unsettled by D155 and D164 and recorded as unsettled by D167, without rewriting those records. Complements D080, D155, D164, and D167. Retention class confirmed against [DATA_RETENTION.md](DATA_RETENTION.md) Structured learning signals / durable workflow intelligence. Broader product work listed under **Not authorized** remains outside this decision; the bounded evidence-carrier slice is the sole implementation authorization here. **Amended by D176 (2026-08-13):** the **Not authorized** prohibitions of Android or web responsibility-selection UX and proposal-review UI are **superseded only to the extent D176 grants** bounded S5 Android Owner proposal lifecycle; D168 remains **Approved** historical and operative law for the responsibility-selection evidence carrier. Web responsibility-selection UX, a public read API for the evidence, exposing the evidence on TaskSuggestion, Task responsibility/assignee/custody columns, Owner TaskAssignment, automatic assignment/handoff/delivery, and every other exclusion D176 does not lift remain unauthorized here
...
**Notes:** Owner decision 2026-08-12 (documentation/governance only). D170's Android capture switch / Android proposal UI / S3.3 prohibitions are **lifted only to the extent** of the S3.3 capture-to-proposal boundaries above — D170 remains **Approved**; its S3.2 authorization and every exclusion this decision does not lift remain intact. Complements D154, D157, D161, D162, D169, D170 without redesigning the interpretation contract or inventing a second proposal model. Domain status reconciliation: [ARCHITECTURE.md](ARCHITECTURE.md), [API_CONTRACT.md](API_CONTRACT.md), [MILESTONES.md](MILESTONES.md), [GLOSSARY.md](GLOSSARY.md), [AI_CONSTITUTION.md](AI_CONSTITUTION.md), [DATA_RETENTION.md](DATA_RETENTION.md), [SECURITY_AND_PRIVACY.md](SECURITY_AND_PRIVACY.md). **Does not begin S3.3 implementation** by itself; the S3.3 slice is the sole implementation authorization granted here, and every item under **Not authorized** remains outside this decision. **Amended by D176 (2026-08-13):** the **Not authorized** prohibitions of proposal approve, responsibility selection UI, Recipient picker (only to selecting one existing active saved Recipient at Accept), proposal edit (only to the narrow S5 wording surface), later proposal-lifecycle implementation, Task creation from a proposal, navigation to a resulting approved Task, and S2 `approvedTaskId` recovery UX are **superseded only to the extent D176 grants** bounded S5 Android Owner proposal lifecycle; D171 remains **Approved** historical and operative law for S3.3. Merge, standalone proposal inbox, Gmail, SMS, reminders, Production activation, and every other exclusion D176 does not lift remain unauthorized here
...
### D174 — Controlled S4.2 authorization: Recipient web brand/UI alignment (presentation only, Recipient-scoped)

**Status:** Approved

**Decision:** **Controlled S4.2 authorization — Recipient web brand/UI alignment:** the Owner explicitly authorizes the second controlled implementation slice of **S4** ([MILESTONES.md](MILESTONES.md) § Forward sequence): **S4.2 — Recipient Web Brand/UI Alignment**, which migrates the external Recipient capability experience at `/c/{token}` onto the canonical Rocket Communicator dark presentation defined by [BRAND.md](../BRAND.md), D172, and D173. D172 and D173 remain operative in full; this record withdraws no clause from either and adds no colour to the palette. **S4.2 is presentation work only.** The Recipient experience must remain **behaviourally identical**: this slice creates no product capability, changes no wording that carries meaning, and removes or adds no action. **Retires exactly one D173 boundary, and only for the surface it named.** D173 preserved the un-migrated Recipient surface deliberately, requiring S4.1 to be additive and Owner-scoped and recording that "Recipient visual migration is a later S4 slice and is **not** authorized here". S4.2 is that slice. Nothing else in D173 is withdrawn: the shared legacy tokens keep their P1.4 values, the Owner-scoped remaps stay Owner-scoped, and the prohibition on globally re-valuing a shared token to make a migration convenient continues to bind. **Recipient-scoped, not global:** `apps/web/app/globals.css` still serves `/` and `/login`, which are **not** in this slice and must remain visually unchanged, so S4.2 must **not** change global element styling, must **not** set `color-scheme: dark` at `:root`, and must **not** re-value `--aicaa-color-ink`, `--aicaa-color-muted`, `--aicaa-color-line`, `--aicaa-color-accent`, `--aicaa-color-paper`, `--aicaa-color-canvas-top`, or `--aicaa-color-accent-glow`. The Recipient dark surface, including its `color-scheme` treatment and its full-viewport background, is established **inside the Recipient stylesheet**, scoped to the Recipient page container that all three Recipient views already share. **Direct role consumption, not token remapping:** the Recipient stylesheet consumes the S4 role tokens by name. It must **not** remap the legacy tokens within its own scope the way the Owner shell does, because the Recipient's five uses of `--aicaa-color-accent` are a decorative rule, a focus ring, a primary fill, a primary border, and an informational rule — five different semantic roles overloaded onto one name — and a blanket remap would repeat exactly the overloading D172 requires S4 to split. Direct consumption also keeps the existing guard that the Recipient stylesheet declares no custom property of its own intact rather than deleted. **Recipient semantic hierarchy:** page background is `--aicaa-color-background`; ordinary panels and controls are `--aicaa-color-surface-raised-solid`; the dialog is a raised surface over `--aicaa-color-scrim-dark`; quiet informational panels are `--aicaa-color-surface-cool`; body and heading text is `--aicaa-color-text`; supporting copy is `--aicaa-color-text-muted`; structural rules and control boundaries are `--aicaa-color-border` and `--aicaa-color-border-strong`; the instruction rule is `--aicaa-color-border-cool`; the informational banner rule is `--aicaa-color-info`; the sole primary action is the dialog's Confirm control, filled `--aicaa-color-primary` with `--aicaa-color-on-primary`; the error banner rule and the Return-to-owner action are `--aicaa-color-destructive`; focus is `--aicaa-color-focus`. **Destructive treatment — the Return-to-owner action:** returning a Task to the Owner ends Recipient access and is the one destructive action on this surface. It ships as **destructive text with a destructive border on a raised surface**, not as a filled `#EF4444` control with white normal-size text: white on filled `#EF4444` measures **3.76:1** and fails the AA normal-text target, exactly as D173 records. The shipped pair is `#EF4444` on `#121216` at **4.97:1**. Its destructive meaning is additionally carried by its label and by the dialog copy that states the link stops working, never by colour alone, and it stays visually distinct from the filled Rocket-red primary confirmation. **Contrast evidence — independently computed for this record using WCAG 2.x sRGB relative luminance, for the pairs S4.2 actually ships:** `#F5F5F7` measures 18.71:1 on `#050506`, 17.16:1 on `#121216`, and 15.99:1 on `#171A21`; `#A1A1AA` measures 7.95:1 on `#050506`, 7.29:1 on `#121216`, and 6.79:1 on `#171A21`; `#EF4444` measures 4.97:1 on `#121216`; `#FFFFFF` on filled `#E10613` measures 4.96:1; `#8FA3BF` measures 7.92:1 on `#050506` and 6.76:1 on `#171A21`; `#D4D4D8` measures 13.78:1 on `#050506`, 12.64:1 on `#121216`, and 11.78:1 on `#171A21` — each clears the AA 4.5:1 normal-text target, and the focus indicator clears the 3:1 non-text target on every surface it appears against. **Subtle text `#7C7C87` measures 4.53:1 on `#121216` but only 4.29:1 on `#18181D` and 4.22:1 on `#171A21`**, so it must not carry normal-size body text on the soft or cool surfaces, consistent with D172 and D173. The **disabled** treatment stays the existing 55% opacity rather than adopting `--aicaa-color-text-disabled`: at 55% over `#121216` the resolved foreground is `#8F8F92` at **5.79:1**, whereas `#52525B` on `#121216` measures 2.42:1, so replacing opacity with the disabled token here would make a submitting dialog _less_ legible. **Borders are structural, and are not the sole cue:** `#2B2B33` measures 1.33:1 and `#454550` 1.98:1 against `#121216`, so neither may be treated as satisfying 3:1 non-text contrast; every Recipient control is identified by its own visible text label and by the focus indicator, not by its boundary. **Touch targets:** the shipped Recipient controls currently resolve to roughly 38–39 px high and do not meet the approximately 44 px target BRAND.md states, so raising Recipient buttons and form controls to `--aicaa-target-min` is authorized as a directly required accessibility correction in this slice. **Preserved without exception:** existing Recipient class names and markup structure; existing Recipient radii, which D172 expressly permits to remain and which must **not** be squared onto the `0` radius tokens; the absence of motion, so no transition, animation, or `@keyframes` may be introduced and the existing guard against them stays; the Recipient type scale, which is not re-valued by this slice; and the rule that status is never communicated by colour alone. **Capability and security behaviour is out of scope and must not change:** capability-token validation, Recipient authorization semantics, `GET` and mutation behaviour, action availability, Task state transitions, route caching and indexing directives, secret handling, telemetry redaction, and audit attribution are all untouched. **No JavaScript or product-logic change is authorized in this slice**, including no new Recipient layout, wrapper, component, prop, class name, or `data-*` attribute; the dark surface must be achieved from the stylesheet alone. If implementation demonstrates that a presentation hook in markup is genuinely unavoidable, it stops and returns for explicit authorization rather than adding one. **Test pinning — deliberately updated, never weakened:** the Recipient guards in `apps/web/__tests__/p1-5-capability-tokens.test.ts` and `apps/web/e2e/specs/p1-5-capability-tokens.spec.ts` pin the pre-migration light presentation, so they must be **re-pointed to equivalent post-migration traceability in the same reviewed slice**, and must never be deleted, loosened, skipped, or bypassed. The literal multiset must be replaced by an equally exact post-migration multiset rather than removed; the per-token reference counts must be replaced by per-role reference counts rather than dropped; the class-name set must remain pinned unchanged, since no class is renamed; the guard that the Recipient stylesheet declares no token of its own must remain; the runtime computed-style guard must continue to resolve each expected colour from the token layer rather than hard-coding a value, and must gain the destructive, primary, and focus pairs the migration introduces. In `apps/web/__tests__/p1-4-tokens.test.ts` the Recipient stylesheet joins the set asserted to contain no bare colour literal, the assertion that global canvas and `color-scheme` rules stay on the legacy light tokens remains **unchanged and green**, and the assertion recording which surfaces still consume the legacy tokens is retargeted to `/` and `/login` without altering a single pinned value. D119's accessibility gate applies unchanged: zero serious or critical automated findings across the Recipient states already scanned, plus the existing keyboard, Escape, and focus-restoration validation of the confirmation dialog. **Legacy shared-token consolidation is deferred, not authorized here:** after S4.2 the legacy tokens survive as the palette of `/`, `/login`, and the Owner-shell remaps, and retiring or renaming them is a separate closeout concern with its own blast radius. Removing, renaming, or re-valuing any legacy shared token is **not** authorized by this record; a wholesale token rename remains unauthorized (D172). **Authorized S4.2 scope:** Recipient stylesheet migration onto the S4 role tokens; the Recipient-scoped dark surface and `color-scheme` treatment; semantic primary, informational, destructive, and focus presentation on the Recipient surface; the directly required Recipient test-expectation updates described above; and the directly required Recipient accessibility corrections, namely touch-target sizing and any contrast pairing this record's evidence shows must change. Nothing beyond what is necessary to bring the Recipient web surface onto the canonical visual system is authorized. **Not authorized:** any Recipient behaviour, capability semantics, authorization, or security change; any JavaScript, TSX, markup, or class-name change; global stylesheet or `:root` changes; re-valuing, renaming, or removing any shared legacy token; migration of `/` or `/login`; Android migration; API/contracts; schema/database; reminder semantics; proposal-lifecycle implementation; responsibility selection; Recipient picker; Merge; Task creation from proposals; logo/favicon or product-name work; shape or radius redesign; typography dependency or type-scale changes; motion or animation; new colours of any kind; Owner Acceptance Week execution; P2.2 entry; Production activation

**Notes:** Owner decision 2026-08-12 (documentation/governance only). Authorizes the second S4 implementation slice, **S4.2 — Recipient Web Brand/UI Alignment**. **Supersedes nothing, and is superseded by nothing.** D172 and D173 both remain operative in full and neither status changes: D172 keeps the status D173 left it with, D173 remains **Approved**, and this record withdraws no clause from either. D173's Recipient-preservation clause was scoped to S4.1 by its own terms and is satisfied rather than withdrawn — S4.1 shipped with the Recipient surface unchanged, and this record is the separate authorization D173 said Recipient migration would require. D173's remaining boundaries continue to bind, in particular that shared legacy tokens are not globally re-valued and that the Owner-scoped remaps are not collapsed. Complements D116, D119, D124, D172, and D173 and amends none of them; `packages/ui` remains a semantic-token layer only and no token is added, renamed, or re-valued by S4.2. D173's warning that the Recipient runtime token guard detects a remap but not a global re-value still holds, which is why the value pins in `apps/web/__tests__/p1-4-tokens.test.ts` remain the evidence of record for token values and the runtime guard remains evidence only of correct role assignment. Contrast values above were computed independently for this record from WCAG 2.x sRGB relative luminance and are consistent with the figures D172 and D173 record. Reconciliation: [MILESTONES.md](MILESTONES.md), [../BRAND.md](../BRAND.md), [REVIEW_CHECKLIST.md](REVIEW_CHECKLIST.md) § Presentation and brand. **Does not itself change any application code, CSS, or test:** the S4.2 slice is the sole implementation authorization granted here, it is implemented and reviewed separately, later S4 slices including Android visual migration and any legacy shared-token consolidation remain separately unauthorized, and every item under **Not authorized** remains outside this decision

### D175 — Controlled S4.3 authorization: Android Owner brand/UI alignment (presentation only, Android-scoped)

**Status:** Approved

**Decision:** **Controlled S4.3 authorization — Android Owner brand/UI alignment:** the Owner explicitly authorizes the third controlled implementation slice of **S4** ([MILESTONES.md](MILESTONES.md) § Forward sequence): **S4.3 — Android Brand/UI Alignment**, which migrates the native Owner Android app onto the canonical Rocket Communicator dark presentation defined by [BRAND.md](../BRAND.md), D172, D173, and D174. D172, D173, and D174 remain operative in full; this record withdraws no clause from any of them and **adds no colour to the palette**. **S4.3 is presentation work only.** The Owner Android experience must remain **behaviourally identical**: this slice creates no product capability, adds or removes no action, changes no wording that carries meaning, and touches no ViewModel, repository, networking, persistence, or capture/proposal logic. **Retires exactly one boundary, and only for the surface it named.** D173 and D174 each recorded that "Android migration" and "Android visual migration" remain later, separately unauthorized slices. S4.3 is that slice. Nothing else in either record is withdrawn: the web token layer is untouched, `packages/ui` remains a semantic-token layer only, the shared legacy web tokens keep their P1.4 values, the Owner-scoped remaps stay Owner-scoped, and the prohibition on globally re-valuing a shared token continues to bind. **Android-scoped, and no Kotlin token generation:** D116 records that Android parity is achieved by **re-implementing documented rules against shared token values, not by sharing code**, and D116's prohibition on Kotlin/cross-platform token generation is **not** lifted here. S4.3 therefore hand-writes the authorized values into the existing platform-local Android colour holder; it must **not** add a generator, a build step, a shared Kotlin module, or a dependency from Android onto `packages/ui`. No web file, no CSS, no `packages/ui` file, and no web test is in scope. **The Android starting condition, recorded so the migration is traceable:** `Theme.kt` calls `lightColorScheme(...)` supplying only **five** of the **thirty-six** Material 3 colour roles that `androidx.compose.material3` 1.3.1 defines — `primary`, `background`, `surface`, `onBackground`, `onSurface` — so **thirty-one** roles resolve to the Material 3 **baseline light** palette. That is not cosmetic: verified against the resolved 1.3.1 artifact, `Card` takes its container from `surfaceContainerHighest` (baseline `#E6E0E9`), `AlertDialog` from `surfaceContainerHigh` (`#ECE6F0`) with supporting text from `onSurfaceVariant` (`#49454F`) and its icon slot from `secondary` (`#625B71`), `OutlinedTextField` takes its resting boundary from `outline` (`#79747E`) and its placeholder and label from `onSurfaceVariant`, `HorizontalDivider` defaults to `outlineVariant` (`#CAC4D0`), error presentation resolves to `error` `#B3261E` / `onError` `#FFFFFF` / `errorContainer` `#F9DEDC` / `onErrorContainer` `#410E0B`, `RadioButton`'s unselected state resolves to `onSurfaceVariant`, and `inversePrimary` resolves to Material's baseline violet `#D0BCFF`. A partial dark migration that sets only the five currently-set roles would therefore ship near-white cards, a near-white dialog, a Material-violet inverse role, and a non-Communicator error red. **Complete Material 3 slot completion is mandatory, and `darkColorScheme()` defaults are not acceptable:** S4.3 must supply **all thirty-six** roles explicitly, because `darkColorScheme()` would introduce Material baseline dark values — violet primaries, `#141218`-family neutrals, `#F2B8B5` error tones — that are outside the authorized palette. **Authorized complete dark mapping.** Primary and its foreground: `primary` `#E10613`, `onPrimary` `#FFFFFF`, `primaryContainer` `#121216`, `onPrimaryContainer` `#F5F5F7`, `inversePrimary` `#E10613`. Non-primary interaction: `secondary` `#8FA3BF`, `onSecondary` `#050506`, `secondaryContainer` `#171A21`, `onSecondaryContainer` `#F5F5F7`. No third accent exists, so the tertiary family takes neutrals: `tertiary` `#A1A1AA`, `onTertiary` `#050506`, `tertiaryContainer` `#18181D`, `onTertiaryContainer` `#F5F5F7`. Foundation: `background` `#050506`, `onBackground` `#F5F5F7`, `surface` `#0B0B0D`, `onSurface` `#F5F5F7`, `surfaceVariant` `#18181D`, `onSurfaceVariant` `#A1A1AA`, `inverseSurface` `#F5F5F7`, `inverseOnSurface` `#050506`. Surface ladder, monotonic and with every rendered consumer named: `surfaceContainerLowest` `#050506`, `surfaceContainerLow` `#0B0B0D`, `surfaceContainer` `#121216`, `surfaceContainerHigh` `#121216` — the `AlertDialog` container, a raised surface exactly as D174 established for the Recipient dialog — and `surfaceContainerHighest` `#18181D`, the `Card` container for the read-only proposal card. Structure: `outline` `#7C7C87`, `outlineVariant` `#2B2B33`, `scrim` `#050506`, `surfaceBright` `#18181D`, `surfaceDim` `#050506`. Error family: `error` `#EF4444`, `onError` `#050506`, `errorContainer` `#18181D`, `onErrorContainer` `#EF4444`. **Elevation tinting is neutralized deliberately:** `surfaceTint` is `#0B0B0D`, equal to `surface`, so Material's `surfaceColorAtElevation` blend can never composite an unauthorized intermediate colour onto a container. Do **not** set `surfaceTint` to `primary`, which is the Material default and would tint raised surfaces red. **`outline` is `#7C7C87`, and this is a deliberate evidence-based assignment, not a palette drift:** Material's `outline` role is the resting boundary of `OutlinedTextField`, which is the sole affordance identifying that control, so WCAG 1.4.11 non-text contrast applies to it. Independently computed using WCAG 2.x sRGB relative luminance, the structural border tokens do **not** clear the 3:1 non-text target on any Android surface — `#454550` measures 2.15:1 on `#050506`, 2.08:1 on `#0B0B0D`, and 1.98:1 on `#121216`, and `#2B2B33` measures 1.45:1, 1.40:1, and 1.33:1 on the same surfaces — exactly as D173 already records. `#7C7C87` measures **4.94:1 on `#050506`**, 4.77:1 on `#0B0B0D`, and 4.53:1 on `#121216`, so it clears the target comfortably. `outlineVariant` keeps the structural border token `#2B2B33` because its only consumer is a decorative row separator, and D173 expressly warns against incorrectly requiring every decorative separator to achieve 3:1. **`onError` is `#050506`, not `#FFFFFF`:** D173 and D174 both record that white on filled `#EF4444` measures **3.76:1** and fails the AA normal-text target. `#050506` on `#EF4444` measures **5.41:1**. The role is not rendered by any shipped Android component, so pinning it to the accessible pair costs nothing and removes a latent inaccessible default; `onErrorContainer` `#EF4444` on `errorContainer` `#18181D` measures 4.70:1. Do **not** ship a filled `#EF4444` control with white normal-size text on Android. **The legacy teal accent is split into distinct roles and is not repointed to Rocket red.** D172 expressly forbids mechanically repointing the legacy teal accent token to `#E10613`, and the six shipped Android light literals — `ink` `#1C1917`, `muted` `#57534E`, `paper` `#F5F5F4`, `accent` `#0F766E`, `critical` `#B91C1C`, `caution` `#B45309` — carry more roles than their names admit. The authorized split is: `accent`-as-Material-`primary` becomes the filled Rocket-red primary action; `accent`-as-Task-status-label and `accent`-as-signed-in-identity become **muted or informational neutral text**, because a status label and an identity line are informational, not actions; `accent`-as-proposal-title becomes **primary text `#F5F5F7`**, because a proposal title is content; `accent`-as-success-banner becomes **success `#22C55E`**; `caution` becomes **warning `#F59E0B`**; `critical` becomes **destructive `#EF4444`**; `paper` becomes **background `#050506`**; `ink` becomes **primary text `#F5F5F7`**; and `muted` keeps its name with the deliberately changed value **`#A1A1AA`**. The five legacy light values that no longer carry a role are **removed rather than left in place**, because leaving them would leave hidden legacy light and teal values in the presentation layer; this is a deliberate value migration of a small platform-local colour holder and is **not** a token-rename project, which remains unauthorized (D172). **Rocket red must not flood Android chrome, and Material 3 will flood it unless stopped.** Verified against the 1.3.1 artifact, Material derives from `primary`: the `TextButton` label, the `OutlinedTextField` focused outline, focused label, and caret, the `RadioButton` selected state, the `CircularProgressIndicator` active indicator, the `AlertDialog` text-action label, and the default text-selection handles. D172 states plainly that `#E10613` is **not** the generic colour for navigation, links, focus, borders, informational chrome, or every interactive element, so S4.3 must supply explicit non-red colours for each of those derivations from the theme layer. Authorized: `TextButton` content is **info `#8FA3BF`** (7.92:1 on `#050506`) for ordinary non-primary actions and **destructive `#EF4444`** (5.41:1 on `#050506`) for the Dismiss action; the `OutlinedTextField` focused boundary, focused label, and caret are the **strong neutral focus indicator `#D4D4D8`** (13.78:1 on `#050506`, clearing the 3:1 non-text target on every Android surface); the `RadioButton` selected state and the `CircularProgressIndicator` are **info `#8FA3BF`**, which D173 authorizes for selected and informational treatment and which clears 3:1 on every surface; and text-selection colours are supplied explicitly rather than inherited from `primary`. These overrides live in the Android theme layer as named role helpers so screens reference roles, not literals. **One filled Rocket-red primary per decision area.** BRAND.md § 7 requires the primary action to be unmistakable and forbids competing red actions in one decision area, and § 6 forbids giving every control equal visual weight. The shipped Android screens currently render up to three co-equal filled `Button`s in a single decision area, so a colour-scheme swap alone would produce three competing Rocket-red actions. S4.3 must therefore assign visual weight deliberately: the filled Rocket-red primary is **Sign in** on the sign-in screen, **Capture** on the authenticated shell, **Save** in capture editing, **Retry** in capture recovery, **Capture another** in the capture result, **Capture** or **Retry** on the Task list, the single state-appropriate status transition (**Start**, **Wait**, or **Resume**) or **Retry** on Task detail, and **Confirm** — including the confirmation dialog's send action — on the assign screen; every other action in those areas recedes to a neutral or informational treatment, and **Dismiss** takes the distinct destructive treatment. This re-ranking is visual weight only: **no action is added, removed, relabelled, reordered in the interaction model, enabled, or disabled differently**, and every existing `testTag` is preserved. **Disabled state: Material's alpha derivation is preserved, and the disabled token is deliberately not adopted on Android.** Independently computed, Material's `onSurface`-derived disabled treatment resolves the disabled `TextButton` and `OutlinedTextField` label to `#606062` at **3.25:1** on `#050506` and `#646466` at **3.33:1** on `#0B0B0D`, whereas forcing `#52525B` would measure **2.64:1** and **2.54:1** on the same surfaces — materially _less_ legible. Neither figure reaches the normal-text target, and neither is required to: WCAG 1.4.3 exempts inactive user-interface components, and this is the same reasoning by which D174 preserved the Recipient's opacity treatment instead of adopting the disabled token. The disabled text token `#52525B` is therefore **not** introduced into the Android theme, and no Android colour role may be set to it. Disabled state is not communicated by colour alone: Compose propagates the disabled state into the accessibility semantics tree from `enabled = false`, and the capture, sign-in, and handoff controls additionally change their label text while submitting. **System UI and launch treatment.** The XML window theme currently inherits `android:Theme.Material.Light.NoActionBar`, so the pre-Compose window background is the platform light background and the app shows a light flash before the dark Compose content draws. Authorized: retarget the window theme to a dark platform parent, set `android:windowBackground` to **`#050506`**, and declare the system-bar icon appearance as light-on-dark at the window level so the very first frame is already dark. Separately, `MainActivity` calls `enableEdgeToEdge()` with no arguments; verified against `androidx.activity` 1.10.1, the default `SystemBarStyle.auto` decides system-bar icon polarity from `Configuration.uiMode` — the **device** night setting — not from the app's Compose theme, so on a device set to light mode the app would request dark status-bar and navigation-bar icons over a `#050506` surface and render them effectively invisible. Authorized: pass explicit dark system-bar styles for both bars so icon polarity follows the app's unconditionally dark presentation rather than the device setting. **No splash branding, launcher-icon, adaptive-icon, or logo work is authorized**, and the placeholder launcher icon is untouched. **Touch targets.** Verified against the 1.3.1 artifact, `ButtonDefaults.MinHeight` is **40 dp**, `Button` and `TextButton` do not apply `minimumInteractiveComponentSize`, and the shipped Android buttons therefore resolve below the approximately 44 px minimum BRAND.md § 7 and § 10 state; `RadioButton` already applies the 48 dp interactive minimum, and the Task list rows already exceed the target. Raising `Button` and `TextButton` to a **44 dp** minimum height is authorized as a directly required accessibility correction, exactly as D174 authorized the equivalent Recipient correction, and it is the **only** geometry change in this slice. **Shape is preserved, and there is nothing to preserve it from.** The Android theme supplies no `Shapes`, no screen declares a `RoundedCornerShape`, and no shape customization exists anywhere in the Android source; that remains true after S4.3. Material 3 default shapes stay, controls must **not** be squared, no custom `Shapes` may be introduced, no rounded-card or pill rule may be established, and no `RoundedCornerShape` may be added for S4 (D172, BRAND.md § 6). **Motion and typography are preserved.** No animation, transition, `AnimatedVisibility`, `Crossfade`, `animate*AsState`, or custom motion may be introduced; native and platform performance is preserved. The existing `AicaaTextStyles` type hierarchy is **not** re-valued, no custom `Typography` is supplied to `MaterialTheme`, no `res/font` directory is created, and **no Android Inter font dependency is added** — Inter remains the corporate direction with migration timing controlled separately (D172, BRAND.md § 5). **Inline colour literals.** Three presentation literals live outside the Android theme layer and all three belong to S4: the Task-detail note body `Color(0xFF44403C)`, the assign-screen informational banner tone `Color(0xFF44403C)`, and the Task-list row separator `Color(0xFFE7E5E4)`. All three are migrated into theme roles — the note body and the informational tone onto muted or informational text roles, and the separator onto the `outlineVariant` structural role, which is also Material's own default for `HorizontalDivider` and therefore removes the override rather than re-valuing it. After S4.3 **no colour literal may remain in an Android Compose screen where a theme role exists**; the sole authorized location for a colour literal is the Android theme layer itself. **Accessibility gate.** Every text pair S4.3 ships must clear the AA 4.5:1 normal-text target **against the surface it actually sits on**, verified per surface rather than against the background alone. Independently computed for this record using WCAG 2.x sRGB relative luminance: `#F5F5F7` measures 18.71:1 on `#050506`, 18.06:1 on `#0B0B0D`, 17.16:1 on `#121216`, and 16.24:1 on `#18181D`; `#A1A1AA` measures 7.95:1, 7.67:1, 7.29:1, and 6.90:1 on the same surfaces; `#8FA3BF` measures 7.92:1, 7.64:1, 7.26:1, and 6.87:1; `#22C55E` 8.94:1 and `#F59E0B` 9.49:1 on `#050506`; `#EF4444` 5.41:1 on `#050506`, 5.23:1 on `#0B0B0D`, 4.97:1 on `#121216`, and 4.70:1 on `#18181D`; `#D4D4D8` 13.78:1 on `#050506`; and `#FFFFFF` on filled `#E10613` 4.96:1. **Subtle text `#7C7C87` measures 4.94:1 on `#050506` and 4.77:1 on `#0B0B0D` but only 4.53:1 on `#121216` and 4.29:1 on `#18181D`**, so it must not carry normal-size body text on the raised, soft, or cool surfaces, consistent with D172, D173, and D174; its authorized S4.3 use is the `outline` control-boundary role, where the applicable target is the 3:1 non-text one. `#E10613` measures 4.11:1 as text on `#050506` and must **not** be used as text or as an icon-only cue; it ships as a filled container with `#FFFFFF`. Status, selection, error, warning, success, and disabled state must remain understandable without colour recognition, using label text, the radio control's own selected shape, position, weight, and the Compose semantics tree; do **not** create a colour-only distinction. **Test and guard plan — the minimum, not a visual-test framework.** Android has weaker token pinning than web, which after this slice becomes the only surface whose authorized values are unpinned, so S4.3 must add local JVM guards that pin: every authorized Android colour value individually; that the Material 3 colour scheme has **all thirty-six** roles explicitly assigned and every assigned value is drawn from the authorized set; that no legacy light or teal value — `#1C1917`, `#57534E`, `#F5F5F4`, `#0F766E`, `#B91C1C`, `#B45309`, `#44403C`, `#E7E5E4` — and no Material baseline value survives anywhere in the Android presentation layer; that `#52525B` is absent, with the reason recorded; that no Android Compose screen contains a colour literal; that the theme supplies no custom `Shapes` and no screen declares a shape; that no motion API and no font resource or `Typography` override is present; and that the XML window theme carries the dark parent and the `#050506` window background. These guards must be **deliberately updated, never deleted, loosened, skipped, or bypassed**, in the same reviewed slice as any later value change, exactly as D172 requires of the web pins. Do **not** build a screenshot, golden-image, or visual-regression framework, add an emulator pipeline, or add a test dependency for S4.3; the existing local instrumentation smoke test stays as-is and out of CI. The existing Compose behaviour tests must keep passing unchanged, because behaviour does not change. **Not authorized:** any Android behaviour, ViewModel, repository, networking, persistence, capture, retry, proposal, or authorization change; any web, `packages/ui`, CSS, or Recipient change; Kotlin or cross-platform token generation, a shared token module, a build step, or an Android dependency on `packages/ui`; re-valuing, renaming, or removing any shared legacy web token; adopting the disabled text token `#52525B` on Android; new colours of any kind; a general decorative accent; proposal-lifecycle implementation; responsibility selection; Recipient picker; Merge; Task creation from proposals; API/contracts; schema/database; auth/security; Gmail/SMS; reminder semantics; copy or product-name changes; logo, launcher-icon, adaptive-icon, or splash branding work; custom typography, type-scale, or font-dependency migration; shape or radius redesign; motion or animation; a token-rename project; a visual-regression test framework; Owner Acceptance Week execution; P2.2 entry; Production activation

**Notes:** Owner decision 2026-08-13 (documentation/governance only). Authorizes the third S4 implementation slice, **S4.3 — Android Brand/UI Alignment**. **Supersedes nothing, and is superseded by nothing.** D172, D173, and D174 all remain operative and no status changes: D172 keeps the status D173 left it with, and D173 and D174 both remain **Approved**. D173's and D174's statements that Android visual migration remains a later, separately unauthorized slice were scoped by their own terms to the slices they authorized and are **satisfied rather than withdrawn** — this record is the separate authorization both anticipated. Their remaining boundaries continue to bind, in particular that shared legacy web tokens are not globally re-valued, that the Owner-scoped remaps are not collapsed, and that `packages/ui` remains a semantic-token layer only. Complements D116 and D124 and amends neither: D116's prohibition on Kotlin and cross-platform token generation is **not** lifted, and D116's recorded cross-platform reality — that Android inherits presentation **rules and values**, not code — is the reason S4.3 hand-writes the authorized values into the existing platform-local Android colour holder. No token is added, renamed, or re-valued in `packages/ui` by S4.3. The palette is unchanged: this record adds no colour and relies only on values D172 and D173 already authorized. Contrast values above were computed independently for this record from WCAG 2.x sRGB relative luminance and are consistent with the figures D172, D173, and D174 record. The Material 3 role derivations, baseline light values, `ButtonDefaults.MinHeight`, and `SystemBarStyle.auto` dark-mode detection cited above were verified against the resolved `androidx.compose.material3` 1.3.1 and `androidx.activity` 1.10.1 artifacts rather than from documentation. Reconciliation: [MILESTONES.md](MILESTONES.md), [../BRAND.md](../BRAND.md), [REVIEW_CHECKLIST.md](REVIEW_CHECKLIST.md) § Presentation and brand. **Does not itself change any Kotlin, XML, or test file:** the S4.3 slice is the sole implementation authorization granted here, it is implemented and reviewed separately, any legacy shared-token consolidation and any Android typography, shape, or motion migration remain separately unauthorized, and every item under **Not authorized** remains outside this decision

### D176 — Controlled S5 authorization: Android Owner proposal lifecycle

**Status:** Approved

**Decision:** **Controlled S5 authorization — Android Owner proposal lifecycle:** the Owner explicitly authorizes **S5 — Android proposal lifecycle** as a canonical roadmap milestone ([MILESTONES.md](MILESTONES.md) § Forward sequence) and authorizes bounded Android implementation of the remaining half of the locked AI-first capture direction after completed S3 (read-only proposals) and completed S4 (presentation). **D171 remains Approved historical and operative law** for S3.3 capture-to-proposal; D171's prohibitions of later proposal-lifecycle implementation, proposal approve, responsibility selection UI, Recipient picker, proposal edit, Task creation from a proposal, navigation to a resulting approved Task, and S2 `approvedTaskId` recovery UX are **lifted only to the extent** this decision grants the bounded S5 Android Owner proposal lifecycle below — they do **not** broadly authorize Merge, a proposal-inbox product, Recipient creation inside Accept, automatic assignment/handoff/delivery, backend redesign, or Production activation. **D168 remains Approved historical and operative law** for the dedicated append-only responsibility-selection evidence carrier; D168's prohibitions of Android responsibility-selection UX and proposal-review UI are lifted only to the same S5 extent. **Authorized product loop:** Android Owner may move from S3's read-only proposals into Capture → Rocket proposes 0..N TaskSuggestions → Owner reviews → Edit / Dismiss / Accept → Accept requires affirmative responsibility selection → canonical Task is created → responsibility-selection evidence is persisted atomically → Android transitions to the resulting Task. **Reuse required — no parallel proposal system:** S5 must reuse the architecture already established by previous Decisions and current repository contracts: the existing `TaskSuggestion` lifecycle; the existing Owner proposal list/detail/approve/edit/dismiss routes; the existing D168 responsibility-selection evidence carrier; the existing atomic approve transaction; the existing `ResponsibilitySelection` contract; the existing active Recipient listing; the existing strong ETag / `If-Match` optimistic-concurrency discipline; the existing nullable `TaskSuggestion.approvedTaskId` recovery mechanism; the existing Android authenticated networking stack (`OwnerApiExecutor` / `OwnerApiRepository`); the existing lightweight Android navigation; and the existing Task detail flow. Do **not** authorize a parallel proposal system, a second approve path, or a second responsibility model. **Accept semantics:** Android Accept is authorized only when the Owner has made an affirmative responsibility selection using the dedicated D168 responsibility concept. The allowed affirmative choices are **Me / Owner** and one **saved Recipient**. Absence of a selection must **never** mean Owner/Me, and must not be inferred from absence of Recipient selection, absence of `TaskAssignment`, absence of handoff, or absence of any responsibility-evidence readback. **Me / Owner** must serialize as the existing Owner responsibility variant (`responsibleParty = owner`). **Saved Recipient** may identify one existing active saved Recipient through the existing Recipient listing; the selection means only that the Owner affirmatively chose that Recipient as the responsible party at acceptance time. Preserve D168 exactly: Recipient responsibility selection must **not** itself create `TaskAssignment`, create `TaskCapability`, create `HandoffAttempt`, send Gmail, send email, activate Recipient access, or constitute completed handoff or delivery. Do **not** repurpose the legacy top-level `recipientId` approval concept. Do **not** require or authorize Recipient creation inside the Accept flow. **Lost-response recovery:** authorize Android recovery through the existing nullable `TaskSuggestion.approvedTaskId`. If approve succeeds but its response is lost or otherwise ambiguous, Android may re-read the canonical suggestion; a non-null `approvedTaskId` is evidence that approval committed; Android may then open the resulting canonical Task. This authorization must **not** introduce approve idempotency, a second server-side recovery state machine, Android local proposal persistence, a local proposal database, or extension of `PendingCaptureStore` into proposal persistence. **Optimistic concurrency:** all Android proposal mutations must reuse the existing strong ETag / `If-Match` discipline. D176 does **not** authorize request-body version substitutes, last-write-wins mutation behaviour, or silent retries against stale proposal state. Stale or conflicting state must be resolved by canonical re-read rather than by inventing another concurrency mechanism. **Edit:** authorize a narrow proposal-editing interaction on or immediately around the existing S3 proposal review surface. For the initial S5 implementation, the intended minimum editable surface is existing proposal summary-point wording, preserving canonical point identity, kind, and order. This record authorizes only the minimum proposal edit surface required by S5; it does **not** permanently freeze all future proposal-editing product design. Do **not** authorize in S5: draft/autosave infrastructure; persistent edit sessions; structural proposal-editor infrastructure; general proposal authoring; deadline editing; priority/urgency editing; reminder editing. **Dismiss:** authorize Android dismissal using the existing proposal-dismiss mutation and existing concurrency semantics. A dismissal-reason UI is **not** required by S5. Do **not** invent additional dismissal persistence. **UI / navigation:** authorize the minimum Android UI necessary to expose this lifecycle on or immediately around the existing S3 proposal review surface. Responsibility selection belongs within the Accept interaction. Do **not** require or authorize a standalone responsibility-selection destination. Preserve the existing lightweight Android navigation architecture. Do **not** introduce Navigation-Compose solely for S5. Successful Accept may transition to the existing canonical Task detail experience. **S5 implementation sequencing (current plan, not immutable architecture):** the current controlled sequence is **S5.1** Android proposal lifecycle client plumbing — canonical suggestion re-read; approve/edit/dismiss networking; existing `If-Match` behaviour; decode `approvedTaskId`; request/response serialization; no user-visible lifecycle actions yet. **S5.2** Accept + responsibility selection + Task transition/recovery — Accept on the existing proposal review UI; affirmative Me / saved Recipient choice; reuse of the active Recipient listing; canonical Task transition; lost-response recovery through `approvedTaskId`. **S5.3** Edit + Dismiss — narrow proposal wording edit; Dismiss; no Merge or expanded proposal-management product. This sequencing is the current implementation plan, subject to review between slices. This decision authorizes the bounded S5 behaviour; it does **not** force unnecessary temporary abstractions if later repository evidence shows two slices can safely collapse. **Not authorized:** Merge in Android; proposal inbox/list-browsing product work beyond what is minimally required for the capture decision loop; Recipient creation inside the Accept flow; automatic assignment/handoff/delivery after Recipient responsibility selection; `TaskAssignment`, capability, or handoff creation as part of responsibility selection; responsibility/assignee/custody fields on `Task`; Owner `TaskAssignment`; a public read API for responsibility-selection evidence; exposing responsibility evidence on `TaskSuggestion` merely for S5 UI; approve idempotency; Android local proposal persistence/database; push/FCM/WorkManager proposal or attention infrastructure; reminders; reminder controls; deadline/urgency follow-through UI; Production AI activation; interpretation feature-flag activation; cron changes; Gmail intake; SMS intake; Gmail Review with Rocket; SMS Share to Rocket; OAW / Owner Acceptance Week; deployment/release activation; dependency on the legacy A6 Gmail suggestion-extraction path; generator/toolchain upgrades; shared-token cleanup; `/` or `/login` visual migration; unrelated S4 visual work

**Notes:** Owner decision 2026-08-13 (documentation/governance only). Establishes **S5 — Android proposal lifecycle** in the canonical roadmap after completed S3 and S4. D172's statement that no S5 is created _here_ remains correct history of that S4 authorization and is not withdrawn; this record is the separate authorization that creates S5. D168, D169, D170, and D171 remain **Approved**; their prohibitions are lifted only to the extent granted above. Complements D080, D154, D155, D161, D164, D168, D169, D170, and D171 without redesigning the proposal, Task, or responsibility-selection architecture and without inventing a second proposal model. Domain status reconciliation: [MILESTONES.md](MILESTONES.md). **Does not begin S5.1 implementation** by itself; the bounded S5 behaviour above is the implementation authorization granted here, no S5 implementation slice is marked complete or started by this record, and every item under **Not authorized** remains outside this decision
