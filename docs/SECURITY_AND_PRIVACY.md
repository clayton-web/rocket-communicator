...
- **SMS/text Review with Rocket (D181, authorized, not implemented):** Owner-granted Notification Listener access is the only Messages intake. Rocket must not request `READ_SMS`, `RECEIVE_SMS`, or `SEND_SMS`, must not become the default SMS handler, and must not use the Telephony inbox as the primary source. Google Messages package(s) must be explicitly allowlisted. Recent eligible notification information remains device-local until the Owner explicitly chooses Review with Rocket; notification arrival must not upload, interpret, or persist communication content on the server. Malformed, empty, unsupported, media-only, group, or otherwise unsafe occurrences fail closed. Existing OTP/financial-alert prompt restrictions remain applicable. Exclude Number stores only a canonical E.164 phone number; audit events must not place the number in `AuditEvent.note`; public mutation responses should avoid unnecessarily echoing it.
