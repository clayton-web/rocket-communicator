...
- **S3.3 / D171 (authorized, not yet implemented):** Android may temporarily persist pending manual-capture retry state — the frozen idempotency tuple including raw capture text — in encrypted local preferences following the existing `PendingHandoffStore` pattern, solely for exact retry/recovery after ambiguous failure or process death. Maximum lifetime **24 hours**; not a capture history, not an offline queue, not independently synced to the backend, and never logged. Server-side raw manual-capture input remains non-retained (D169, D170, D162).
