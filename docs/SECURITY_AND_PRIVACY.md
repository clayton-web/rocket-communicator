...
- **S3.3 / D171 (implemented):** Android temporarily persists pending manual-capture retry state — the frozen idempotency tuple including raw capture text — in encrypted local preferences following the existing `PendingHandoffStore` pattern, solely for exact retry/recovery after ambiguous failure or process death. Storage fails closed when encrypted preferences cannot be initialized, so raw capture text never reaches an unencrypted file. Maximum lifetime **24 hours**; not a capture history, not an offline queue, not independently synced to the backend, and never logged. The read-only proposal result lives only in ViewModel memory and is never written to disk. Server-side raw manual-capture input remains non-retained (D169, D170, D162).
...
- **SMS/text Review with Rocket (D181, device-local intake, server Review, and Android Review invocation implemented; Exclude Number not implemented):** Owner-granted Notification Listener access is the only Messages intake. Rocket must not request `READ_SMS`, `RECEIVE_SMS`, or `SEND_SMS`, must not become the default SMS handler, and must not use the Telephony inbox as the primary source. Google Messages package(s) must be explicitly allowlisted (`com.google.android.apps.messaging`). Recent eligible notification information remains device-local, in-memory, and bounded (at most 25 items or 24 hours) until the Owner explicitly chooses Review with Rocket; notification arrival must not upload, interpret, or persist communication content on the server. Selected text is persisted on the server only at the explicit Owner-authenticated `POST /api/v1/messages/reviews` boundary, only as `TemporaryCommunicationExcerpt`. Resulting proposals use the existing S5 proposal architecture. Malformed, empty, unsupported, media-only, group, or otherwise unsafe occurrences fail closed. Existing OTP/financial-alert prompt restrictions remain applicable. Exclude Number is not implemented in this slice; when authorized later it stores only a canonical E.164 phone number; audit events must not place the number in `AuditEvent.note`; public mutation responses should avoid unnecessarily echoing it.
