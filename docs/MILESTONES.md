...
**Not built:** Messages capture, call prompts, the voice pipeline, retention workers, learning signals, and private-deployment hardening. **S5 — Android proposal lifecycle** is authorized by **D176** and **not yet complete**. **S5.1** (Android proposal lifecycle client plumbing: canonical suggestion re-read, approve/edit/dismiss networking, existing `If-Match` behaviour, `approvedTaskId` decode, request/response serialization) is **implemented** with no user-visible lifecycle actions; the S3 proposal screen remains read-only. **S5.2** and **S5.3** remain unimplemented: Owner review of S3's read-only proposals, Accept with affirmative Me / saved-Recipient responsibility selection, narrow proposal wording edit, Dismiss, canonical Task creation from a proposal, and `approvedTaskId` Android recovery. Merge, proposal inbox/list-browsing beyond the capture decision loop, Recipient creation inside Accept, automatic assignment/handoff/delivery, and the remaining D176 exclusions remain separately unauthorized. Gmail Review with Rocket, SMS Share to Rocket, reminders enablement, the attention system, and exclusions remain later work.

**Next:** **S5 — Android proposal lifecycle** (authorized by D176; **S5.1** implemented, S5.2 and S5.3 not started), then product surfaces, and Owner Acceptance Week (deferred under D159). See [Forward sequence](#forward-sequence).
...
3. **S5 — Android proposal lifecycle** — authorized by **D176**; **implementation not yet complete.** The remaining half of the locked AI-first direction: Owner review of S3's read-only proposals, Accept with affirmative responsibility selection (Me / Owner or a saved Recipient), atomic canonical Task creation with D168 responsibility-selection evidence, transition to the resulting Task, lost-response recovery through `approvedTaskId`, narrow proposal wording edit, and Dismiss. **Current implementation plan**, subject to review between slices: **S5.1** Android proposal lifecycle client plumbing is **implemented** (canonical suggestion re-read, approve/edit/dismiss networking, existing `If-Match` behaviour, decode `approvedTaskId`, request/response serialization; no user-visible lifecycle actions); **S5.2** Accept + responsibility selection + Task transition/recovery (Accept on the existing proposal review UI, affirmative Me / saved Recipient choice, reuse of the active Recipient listing, canonical Task transition, lost-response recovery through `approvedTaskId`) is not started; **S5.3** Edit + Dismiss (narrow proposal wording edit preserving canonical point identity/kind/order, Dismiss using the existing dismiss mutation; no Merge or expanded proposal-management product) is not started. The slice breakdown is the current plan, not an immutable architectural requirement; two slices may collapse if later repository evidence shows that is safe. S5 remains bounded by D176: no parallel proposal system; no Merge; no proposal inbox/list-browsing product beyond the capture decision loop; no Recipient creation inside Accept; no automatic assignment, handoff, or delivery; no Android local proposal persistence; no approve idempotency; no Navigation-Compose; no Production activation; and the remaining exclusions recorded in D176.
