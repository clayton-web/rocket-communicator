...
1. **P1** — Application Experience Foundation (distinct milestone; **not** part of A8 and not folded into it). **P1.0 decision lock is complete** (D111–D120); **P1.1 is implemented**; P1.2–P1.5 are authorized and not started. Scope: [P1](#p1--application-experience-foundation).
...
**Status:** **P1.0 documentation Decision Lock complete** — D111–D120. **P1.1 Minimal Observability and Unified Correlation — implemented; architecture, security, and regression review passed.** Production validation against the P1.1 baseline remains **P1.5** (D119). P1.2–P1.5 authorized, not started. There is still no application shell, `packages/ui`, browser test harness, loading state, global error fallback, or not-found state (those are later slices).
