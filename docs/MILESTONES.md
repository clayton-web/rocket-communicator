...
**Built and deliberately inert:** the reminder and Owner-notification engines. The scheduling domain, persistence, Owner reminder APIs, occurrence processing, real Gmail transport, and the Owner attention surfaces are all deployed, but every feature flag is absent and no scheduler invokes either worker, so nothing sends. Enabling any of it requires its own authorization. **S3.1 shared interpretation** joins them: the interpretation application service and its atomic occurrence/proposal persistence are implemented, but no route, Android surface, worker, or cron reaches them, provider composition is default closed, and no Production activation has been authorized.

**Not built:** Messages capture, call prompts, the voice pipeline, retention workers, learning signals, and private-deployment hardening. **S3.2 Owner manual-capture HTTP/API** is **authorized by D170** and not yet implemented.

**Next:** implement authorized **S3.2** (D170) — Owner manual-capture HTTP route + generated public contracts — then later controlled S3 slices, product surfaces, and Owner Acceptance Week (deferred under D159). See [Forward sequence](#forward-sequence).
