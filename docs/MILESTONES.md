...
**Not built:** Messages capture, call prompts, the voice pipeline, retention workers, learning signals, and private-deployment hardening. **S5 — Android proposal lifecycle** is authorized by **D176**. **S5.1** (Android proposal lifecycle client plumbing), **S5.2** (Accept + affirmative Me / saved-Recipient responsibility selection + canonical Task transition + `approvedTaskId` lost-response recovery), and **S5.3** (narrow proposal wording edit and Dismiss) are **implemented**. S5 implementation slices are delivered; final S5 milestone closeout remains a separate completion review. Merge, proposal inbox/list-browsing beyond the capture decision loop, Recipient creation inside Accept, automatic assignment/handoff/delivery, and the remaining D176 exclusions remain separately unauthorized. Gmail Review with Rocket, SMS Share to Rocket, reminders enablement, the attention system, and exclusions remain later work.

**Next:** read-only **S5 completion audit/closeout** (S5.1–S5.3 implemented; that review is separate from this implementation record), then product surfaces, and Owner Acceptance Week (deferred under D159). See [Forward sequence](#forward-sequence).
