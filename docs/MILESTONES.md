...
**Next:** **S4 — Rocket Communicator Brand/UI Alignment** (D172, presentation only), then later controlled Android proposal-lifecycle work (separately unauthorized), product surfaces, and Owner Acceptance Week (deferred under D159). See [Forward sequence](#forward-sequence).
...
2. **S4 — Rocket Communicator Brand/UI Alignment** — established and authorized by **D172**, sequenced deliberately **ahead of** Owner Acceptance Week and P2.2. Its purpose is to bring shipped Rocket Communicator presentation into alignment with [BRAND.md](../BRAND.md): the canonical dark colour system, text hierarchy, surfaces and borders, neutral focus treatment, and the primary-versus-destructive distinction. **Presentation only.** S4 authorizes no proposal Accept/Edit/Dismiss implementation, no responsibility-selection implementation, no Task creation from a proposal, no Merge, no Recipient-picker product work, no new interpretation behaviour, and no other deferred product capability; it establishes no shape, radius, motion, or font-dependency migration. D172 records the milestone and its presentation decisions; each S4 implementation slice is separately authorized and reviewed. **S4.1 — Owner Web Visual Foundation** is the first such slice, authorized by **D173**: additive Owner-scoped dark presentation on the Owner web surface, with the un-migrated Recipient `/c/{token}` experience deliberately preserved visually and functionally. **S4.2 — Recipient Web Brand/UI Alignment** is the second, authorized by **D174**: the external `/c/{token}` experience moves onto the same canonical dark presentation, Recipient-scoped and behaviourally identical, with global styling, `/`, `/login`, and every shared legacy token value left untouched. **S4.3 — Android Brand/UI Alignment** is the third, authorized by **D175**: the native Owner Android app moves onto the same canonical dark presentation, Android-scoped and behaviourally identical, completing every Material 3 colour role explicitly so no baseline light default survives, splitting the legacy teal accent into primary, informational, semantic, and structural roles rather than repointing it to Rocket red, and leaving shape, motion, typography, the web token layer, and the launcher icon untouched. Any legacy shared-token consolidation and any Android typography, shape, or motion migration remain later, separately unauthorized slices. **No S5 exists.**
3. **Owner Acceptance Week** — formal product gate (D142), **deferred and must not be executed** (D159).
4. **P2.2 — Remove Friction** — not started, not authorized.
5. **A8 operational enablement** — capture observation, then notification delivery, then reminder delivery; each separately authorized.
6. **A10+** — Messages capture, call prompts, voice pipeline, retention workers, learning signals, hardening.
