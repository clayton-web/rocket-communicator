...
**Not built:** Messages capture, call prompts, the voice pipeline, retention workers, learning signals, and private-deployment hardening. **S3.3 Android capture-to-proposal integration** is **authorized by D171** and **implemented**: the **S3.3a** foundation (manual-capture wire DTOs, the `POST /api/v1/manual-captures` repository call over the existing Owner networking stack, and the encrypted 24-hour pending-capture retry store) plus the **S3.3b** switch — the Owner Capture screen now sends manual capture to that route, shows the returned 0..N proposals **read-only**, and offers explicit Retry / Discard recovery for an unresolved pending capture. The legacy direct-create Android code remains present but unreachable from the Capture UI, and the backend `POST /api/v1/tasks` endpoint is unchanged. **Full proposal lifecycle** — approve, dismiss, edit, merge, responsibility selection, Recipient picker, proposal inbox, Task creation from a proposal — remains **unimplemented and separately unauthorized**, and no Production activation occurred.
...
1. **AI-first capture and interpretation** — the direction locked by D154, D156, D157, and D164: source → shared interpretation → zero or more proposals → Owner review → accept and choose the responsible person (Owner or Recipient) → canonical Task → follow-through. **D169** authorizes controlled S3 implementation of that path: shared interpretation application service + Owner manual capture, reusing the existing `packages/ai` contract, `InterpretationRun` foundation, and canonical `TaskSuggestion` lifecycle. **S3.1** (first authorized slice) is **implemented as backend infrastructure**: shared interpretation wired to canonical persistence, reachable only through the S3.2 Owner route below, with **no** Android product reachability and no Production activation. Earlier “paused / unauthorized to resume” language for `packages/ai` application wiring was a correct historical gate and is superseded only within D169’s boundaries. **D170** authorized the next controlled slice, **S3.2**, which is now **implemented**: Owner-authenticated `POST /api/v1/manual-captures` plus generated public contracts call the existing S3.1 service with server-fixed `owner_manual_capture`, returning canonical proposals without interpretation provenance. **D171** authorized the next controlled slice, **S3.3**, which is now **implemented**: Android capture switched from legacy direct-create onto that route and displays returned proposals **read-only**, with durable encrypted device-side retry identity; no full proposal-lifecycle UI and no Production activation. S3 for the manual Owner capture path is therefore complete end to end — Android capture → shared interpretation → canonical TaskSuggestions → Android read-only proposal result — while proposal approval, responsibility selection, and every other lifecycle action remain unimplemented and separately unauthorized. Implementation must evolve the one shared proposal path and the one shared interpretation capability ([ARCHITECTURE.md § Ownership and reuse map](ARCHITECTURE.md#ownership-and-reuse-map)), not create a second one. Gmail/SMS producers, full Android proposal-lifecycle UI, generic arbitrary-source interpretation APIs, and Production activation remain separately unauthorized.
