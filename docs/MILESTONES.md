...
**Current:** **A7 is CLOSED** — Gmail forwarding and assignment email are **Production-operational**. A7.1–A7.8 shipped and the full production E2E passed on production SHA `8da353692c39484467f8f4651acf101fa172f4e8` (both delivery paths, Recipient capability completion, Owner-visible notes). Completion tag: `v0.7.0-a7-complete`. A7.0 decisions remain locked (D086–D094). **A8.0 documentation Decision Lock** is recorded (D095–D101) and is now partly superseded. **A8.1 documentation Decision Lock** is recorded (**D102–D110**): A8 is revised to a **due-date-driven** reminder model under a narrow constitutional exception. A8 implementation is **not** started — no A8 code, schema, migration, contract, environment configuration, scheduler, or UI exists — and requires explicit authorization. **P1.0 documentation Decision Lock** is recorded (**D111–D120**). **P1.1 Minimal Observability and Unified Correlation is implemented** (baseline: [P1_1_BASELINE.md](P1_1_BASELINE.md)); it is **not** production-validated against that baseline, which is P1.5 work. **P1.2 Browser Verification Harness is implemented** with **local evidence only** ([P1_2_BROWSER_HARNESS.md](P1_2_BROWSER_HARNESS.md)). **P1.3 Request and render reliability is implemented, pending architectural review**, with **local evidence only** ([P1_3_EVIDENCE.md](P1_3_EVIDENCE.md)). **P1.4 Owner shell, constrained presentation foundation, organization-timezone display, and the Owner attention / operational-status destination — complete and production-validated** ([P1_4_EVIDENCE.md](P1_4_EVIDENCE.md)). P1.5 is not started. A6 Application Suggestion Engine remains **CLOSED** in Production (tag `v0.6.0-a6-complete`). A5 Gmail connection and polling remains **closed and healthy**. Milestone identifiers are unchanged: **A7 → A8 → A9** (no early separate A9.0). **P1** (Application Experience Foundation) is a distinct milestone sequenced before the remaining A8 implementation slices and is **not** folded into A8; see [Delivery sequence](#delivery-sequence). Handoff items deliberately deferred out of A7 are listed under [A7 deferred backlog](#a7-deferred-backlog-not-a-milestone).
...
1. **P1** — Application Experience Foundation (distinct milestone; **not** part of A8 and not folded into it). **P1.0 decision lock is complete** (D111–D120); **P1.1 and P1.2 are implemented**; **P1.3 is implemented, pending architectural review**; **P1.4 is complete and production-validated**; P1.5 is authorized and not started. Scope: [P1](#p1--application-experience-foundation).
...
**Status:** **P1.0 documentation Decision Lock complete** — D111–D120. **P1.1 Minimal Observability and Unified Correlation — implemented; architecture, security, and regression review passed.** Production validation against the P1.1 baseline remains **P1.5** (D119). **P1.2 Browser Verification Harness — implemented, pending architectural review; executable locally only** ([P1_2_BROWSER_HARNESS.md](P1_2_BROWSER_HARNESS.md)). **P1.3 Request and render reliability — implemented, pending architectural review; local evidence only** ([P1_3_EVIDENCE.md](P1_3_EVIDENCE.md)). **P1.4 Owner shell, constrained presentation foundation, organization-timezone display, and the Owner attention / operational-status destination — complete and production-validated** ([P1_4_EVIDENCE.md](P1_4_EVIDENCE.md)). P1.5 authorized, not started. **P1 overall remains open.** The Owner application shell and a tokens-only `packages/ui` now exist (P1.4); there is still **no** global error fallback, not-found state, or connectivity UI (those are P1.5). P1.3 added **only** the two minimal route loading boundaries D112 requires, and P1.4 moved them inside the shell so chrome persists across a pending navigation (see [Loading-state ownership](#loading-state-ownership-across-p13-p14-and-p15)).
...
| **P1.5** | Boundary completion, accessibility verification, connectivity feedback, and production validation against the P1.1 baseline (D112, D119). **Input from P1.4 production validation:** unauthenticated `/tasks` briefly renders identity-independent Owner chrome before its loading-boundary redirect completes — consider whether authentication gating should move earlier (proxy or another route boundary), weighed against D112 loading-boundary behaviour and D119 authentication-count guarantees. Not started. |
...
| **P1.5** | Comprehensive boundary completion — segment error boundaries, global error fallback, styled not-found, lost-connectivity feedback — plus accessibility verification and production validation against the P1.1 baseline (D112, D119).                                                                                                                                                                                                            |

`/c/{token}` loading presentation remains **deferred**: the capability surface is touched last, in P1.5. P1.3 deliberately added no loading file for it.

Local performance evidence recorded in P1.3 ([P1_3_EVIDENCE.md](P1_3_EVIDENCE.md)) is **not** production evidence and does not satisfy the D119 production-validation criterion, which stays with P1.5.
...
- [ ] **Browser-level critical-journey coverage** for Owner sign-in, Task list, Task detail, handoff confirmation, and the Recipient capability journey, running as a separate job (D119) — **partly met by P1.2:** sign-in, Task list, Task detail, and the Recipient capability journey are covered locally; **handoff confirmation remains uncovered** because Gmail is excluded from P1.2 ([P1_2_BROWSER_HARNESS.md](P1_2_BROWSER_HARNESS.md))
