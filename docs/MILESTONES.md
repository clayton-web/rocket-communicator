...
Findings from the A8.3a critical architecture audit that were **deliberately not** fixed in A8.3a or its remediation. None blocks A8.3b, because none is reachable without a worker; each must be resolved before anything can send email. They are listed here rather than described in prose so they cannot quietly disappear.
