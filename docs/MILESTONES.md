...
**Not built:** Messages capture, call prompts, the voice pipeline, retention workers, learning signals, and private-deployment hardening. **Full proposal lifecycle** — approve, dismiss, edit, merge, responsibility selection, Recipient picker, proposal inbox, Task creation from a proposal, and `approvedTaskId` Android recovery/navigation — remains **unimplemented and separately unauthorized**. Gmail Review with Rocket, SMS Share to Rocket, reminders enablement, the attention system, and exclusions remain later work.

**Next:** **S5 — Android proposal lifecycle** (separately unauthorized), then product surfaces, and Owner Acceptance Week (deferred under D159). See [Forward sequence](#forward-sequence).
...
3. **S5 — Android proposal lifecycle** — sequencing only; **not started, not authorized.** The remaining half of the locked AI-first direction — Owner review, accept, responsibility selection, and every other proposal-lifecycle action — still needs its own authorization. This closeout does not begin it.
