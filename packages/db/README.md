...
- **The only producer** is `persistInterpretationOccurrence` (D169 S3.1), which writes one occurrence and its 0..N proposals in one transaction and derives the outcome from the proposal set. The AI call happens before the transaction opens, so no database transaction is held across a provider call. The application service that calls it is reached only by the Owner-authenticated S3.2 route `POST /api/v1/manual-captures` (**D170**, implemented); no worker, cron, or Android surface reaches it.
