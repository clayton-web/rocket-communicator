...
- **The only producer** is `persistInterpretationOccurrence` (D169 S3.1), which writes one occurrence and its 0..N proposals in one transaction and derives the outcome from the proposal set. The AI call happens before the transaction opens, so no database transaction is held across a provider call. The application service that calls it is backend-only today — no route, worker, or cron reaches it. **D170** authorizes S3.2 Owner `POST /api/v1/manual-captures` as the first HTTP adapter (**authorized, not yet implemented**).
