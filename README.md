...
| A8.2 reminder domain logic       | **Complete** (D127) — local-calendar, timezone, and pure scheduling functions in `packages/domain/src/reminders/`; called by the Owner reminder APIs and the Task-lifecycle wiring, never by a worker (none exists)                                                                                  |
...
| Next                             | **A8.4a** reminder scheduler and delivery behind a disabled production feature flag — **gated** until the lifecycle remediation passes focused re-audit ([MILESTONES](docs/MILESTONES.md))                                                                                                           |
...
**Local Postgres via Docker** (Prisma migrate verification, future concurrency / worker tests): `pnpm db:docker:up` then `pnpm db:migrate:local`. Loopback-only on port **5433**; details in [packages/db/README.md](packages/db/README.md). Ordinary Vitest still uses PGlite and does not need Docker.
