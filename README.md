...
| A8.3b Owner reminder APIs        | **Complete** — `GET`/`PUT`/`DELETE /api/v1/tasks/{taskId}/reminder`, OpenAPI and generated contracts; **migration not applied in Production**; no worker, scheduler, cron, flag, email path, or UI                                                      |
| Next                             | **A8.4** reminder scheduler and delivery behind a disabled production feature flag, then the remaining A8 slices ([MILESTONES](docs/MILESTONES.md))                                                                                                     |
