...
| A8.4a worker-safety foundation   | **Complete** — durable occurrence claims with expiry and fencing, expired-claim recovery, deliveries that survive lifecycle races, a coherent Owner `GET`, a global bounded due scan, and `POST /api/v1/internal/reminders/process` **dark and disabled**; **fake transport only** — no Gmail, no cron job, no production flag, no UI |
