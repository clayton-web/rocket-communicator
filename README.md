...
| A8.4a worker-safety foundation   | **Implemented, audited, remediated — awaiting approval** — durable occurrence claims with expiry and fencing, expired-claim recovery, deliveries that survive lifecycle races, a coherent Owner `GET`, a global bounded due scan, and `POST /api/v1/internal/reminders/process` **built dark, disabled, and never deployed**; **fake transport only** — no Gmail, no cron job, no production flag, no UI |
| Next                             | **A8.4b** real Gmail reminder delivery behind the same disabled flag — **gated** on the capability-link envelope decision and the non-success limit ([MILESTONES](docs/MILESTONES.md))                                                                                                                                                                                                                   |
