import { editOwnerSuggestion, parseEditSuggestionBody } from '@/lib/suggestions';
...
export async function POST(
  ...
  return runOwnerTaskRoute(request, async (ctx) => {
    ...
    const parsed = parseEditSuggestionBody(object.value);
    if (!parsed.ok) {
      return parsed.response;
    }
    const result = await editOwnerSuggestion({
      db: ctx.db,
      owner: ctx.owner,
      suggestionId,
      now: ctx.now,
      ...
      proposedRecipientId: parsed.value.proposedRecipientId,
      proposedDueAt: parsed.value.proposedDueAt,
      proposedPriority: parsed.value.proposedPriority,
    });
