import { markCapabilityTaskWaiting } from '@/lib/capability';
...
import { parseWaitingBody } from '@/lib/tasks/validate-body';
...
export async function POST(
  ...
  return runRecipientCapabilityRoute(request, context.params, async (ctx) => {
    ...
    const parsed = parseWaitingBody(object.value);
    if (!parsed.ok) {
      return parsed.response;
    }
    const result = await markCapabilityTaskWaiting({
      db: ctx.db,
      ...
      waitingUntil: parsed.value.waitingUntil,
