...
describe('D181 Google Messages shared interpretation source', () => {
  ...
  it('does not change manual or Gmail fingerprint canonical forms', () => {
    ...
    const gmail = computeInterpretationRequestFingerprint({
      organizationId: org,
      sourceKind: 'gmail',
      rawInput,
      capturedAt,
      timezone: null,
      gmailOccurrenceId: 'evt_s7',
    });
    const gmailWithUnusedMessagesId = computeInterpretationRequestFingerprint({
      organizationId: org,
      sourceKind: 'gmail',
      rawInput,
      capturedAt,
    ...
    expect(gmailWithUnusedMessagesId).toBe(gmail);
  });
