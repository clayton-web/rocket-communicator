...
export interface InterpretationFingerprintInputs {
   ...
   * `sourceKind = google_messages`. Must stay absent for manual capture so those
   * fingerprints do not change (D181).
   */
...
export function canonicalizeInterpretationRequest(inputs: InterpretationFingerprintInputs): string {
  ...
  // Appended only for Google Messages so the manual-capture canonical form stays byte-identical.
