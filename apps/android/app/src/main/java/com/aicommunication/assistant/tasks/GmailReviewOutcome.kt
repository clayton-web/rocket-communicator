...
/**
 * Classification of a Gmail Review `OwnerApiResult` (S7, D161 / D179).
 *
 * Ambiguous outcomes keep the in-memory attempt so Retry reuses the same Idempotency-Key.
 * Terminal rejections clear it so a later explicit Review with Rocket is a new attempt.
 */
enum class GmailReviewOutcome(val preservesAttempt: Boolean) {
    SUCCESS(false),
    ...
    companion object {
        fun classify(result: OwnerApiResult<*>): GmailReviewOutcome = when (result) {
         ...
         */
        private fun classifyHttp(error: OwnerApiResult.HttpError): GmailReviewOutcome = when {
